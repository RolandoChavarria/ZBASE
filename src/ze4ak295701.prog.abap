...
* Generated by SAP_LOCAL_DOWNPORT_ASSISTANT Version 5.70 on 31.10.2016 with option ALL

* The individual coding starts at 'FORM update_01.' ( line 3.516 )
...
CONSTANTS: activation_mode VALUE '2',      "1 = one step, 2 = two step
           ...
           sapbasis TYPE sy-saprl VALUE '701 ',  "SAP_BASIS release for which the report was generated; in lower releases it might get problems
          ...
          'SHI6''ECTD''ELEM''SICF''SRFR''SRFF''SAJT''SAJC''IOBJ'.
...
CLASS lcl_wb DEFINITION.
  PUBLIC SECTION.
    ...
    CLASS-METHODS add_shlp_to_table
      IMPORTING
        i_tabname   TYPE tabname
        i_fieldname TYPE fieldname
        it_dd36m    TYPE tt_dd36m.
...
*      INSERT INITIAL LINE INTO lt_dd03p INDEX l_tabix ASSIGNING <ls_dd03p>.
  ...
  METHOD add_shlp_to_table.
    ...
    DATA: ls_dd02v      TYPE dd02v,
             ...
             lt_dd36m      TYPE TABLE OF dd36m,
             ls_dd36m      TYPE dd36m,
             lt_dd36m_comp TYPE TABLE OF dd36m,
             ls_tabname    LIKE LINE OF gr_tabname VALUE 'IEQ'.

    CALL METHOD set_context
      EXPORTING
        i_langu    = space
        i_text     = 'Assign Search Help to Table'          "#EC NOTEXT
        i_object   = 'TABD'
        i_obj_name = i_tabname.
...
*       GOTSTATE      = GOTSTATE
    ...
    IF lt_dd36m_comp EQ it_dd36m.
      "shlp assignment already like requested
      MESSAGE i666(01) WITH 'No need to change' 'search help assignment of table'  i_tabname INTO sy-lisel. "#EC *
      CALL METHOD log_message.
      RETURN.
    ENDIF.
    ...
    READ TABLE it_dd36m INDEX 1 INTO ls_dd36m.
    IF sy-subrc EQ 0.
      CLEAR ls_dd35v.
      ls_dd35v-tabname    = i_tabname.
      ls_dd35v-fieldname  = i_fieldname.
      ls_dd35v-shlpname   = ls_dd36m-shlpname.
      APPEND          ls_dd35v TO lt_dd35v.
      APPEND LINES OF it_dd36m TO lt_dd36m.
    ELSE.
      "shlp assignment will be removed
      MESSAGE i666(01) WITH 'Search help assignment' 'removed from table' i_tabname INTO sy-lisel. "#EC *
      CALL METHOD log_message.
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ...
      RETURN.
    ELSE.
      MESSAGE i666(01) WITH 'Search help assignment' 'successfully updated' INTO sy-lisel. "#EC *
      CALL METHOD log_message.
    ENDIF.
...
FORM update_01. " TABD CVIS_VEND_CT_REL_KEY ( EN , source: E4A )
  ...
  CALL METHOD lcl_wb=>create_table     "#EC *
    EXPORTING
      i_tabname    = 'CVIS_VEND_CT_REL_KEY'
      i_devclass   = 'CVI_MAPPING'
      i_exclass    = '1'
      i_authclass  = '00'
      i_langu      = 'E'
      i_ddtext     = 'Key Fields Assignment Vendor contact Person - BP Rel'.

  CLEAR: ls_dd08v, lt_dd05m.
  CALL METHOD lcl_wb=>add_field_to_table    "#EC *
    EXPORTING
      i_tabname   = 'CVIS_VEND_CT_REL_KEY'
      i_fieldname = 'PARTNER_GUID'
      i_rollname  = 'BU_PARTNER_GUID'
      i_context   = 'CVIS_VEND_CT_REL_KEY'.

  CLEAR: ls_dd08v, lt_dd05m.
  CALL METHOD lcl_wb=>add_field_to_table    "#EC *
    EXPORTING
      i_tabname   = 'CVIS_VEND_CT_REL_KEY'
      i_fieldname = 'PERSON_GUID'
      i_rollname  = 'BU_PARTNER_GUID'
      i_context   = 'PARTNER_GUID'.

  CLEAR: ls_dd08v, lt_dd05m.
  CALL METHOD lcl_wb=>add_field_to_table    "#EC *
    EXPORTING
      i_tabname   = 'CVIS_VEND_CT_REL_KEY'
      i_fieldname = 'VENDOR_CONT'
      i_rollname  = 'PARNR'
      i_context   = 'PERSON_GUID'.

ENDFORM.

FORM update_02. " TABD CVI_VEND_CT_LINK ( EN , source: E4A )
  ...
  CALL METHOD lcl_wb=>create_table     "#EC *
    EXPORTING
      ...
      i_tabart     = 'APPL0'
      i_langu      = 'E'
      i_ddtext     = 'Connec. Between Relationship + Activity Partner for Vendor'.

  CLEAR: ls_dd08v, lt_dd05m.
  ls_dd08v-tabname               = 'CVI_VEND_CT_LINK'.
  ls_dd08v-fieldname             = 'CLIENT'.
  ls_dd08v-tabname               = 'CVI_VEND_CT_LINK'.
  ls_dd08v-fieldname             = 'CLIENT'.
  ls_dd08v-checktable            = 'T000'.

  CLEAR ls_dd05m.
  ls_dd05m-tabname               = 'CVI_VEND_CT_LINK'.
  ls_dd05m-fieldname             = 'CLIENT'.
  ls_dd05m-fortable              = 'CVI_VEND_CT_LINK'.
  ls_dd05m-forkey                = 'CLIENT'.
  ...
  CLEAR ls_dd36.
  ls_dd36-tabname                = 'CVI_VEND_CT_LINK'.
  ls_dd36-fieldname              = 'CLIENT'.
  ls_dd36-shlpname               = 'H_T000'.
  ls_dd36-shlpfield              = 'MANDT'.
  ls_dd36-shtable                = 'CVI_VEND_CT_LINK'.
  ls_dd36-shfield                = 'CLIENT'.
  APPEND ls_dd36 TO lt_dd36.

  CALL METHOD lcl_wb=>add_shlp_to_table   "#EC *
    EXPORTING
      i_tabname   = 'CVI_VEND_CT_LINK'
      i_fieldname = 'CLIENT'
      it_dd36m    = lt_dd36.
...
FORM update_03. " TABT CVI_VEND_CT_LINK ( EN , source: E4A )
...
FORM update_04. " TTYD CVIS_VEND_CT_LINK_T ( EN , source: E4A )
  DATA: ls_dd42v TYPE dd42v, lt_dd42v TYPE TABLE OF dd42v.

  CLEAR lt_dd42v.
  CALL METHOD lcl_wb=>create_table_type     "#EC *
    EXPORTING
      i_typename   = 'CVIS_VEND_CT_LINK_T'
      i_devclass   = 'CVI_MAPPING'
      i_langu      = 'E'
      i_ddtext     = 'Connec. Between Relationship + Activity Partner for Vendor'
      i_rowtype    = 'CVI_VEND_CT_LINK'
      i_accessmode = 'T'
      i_keydef     = 'D'
      i_keykind    = 'N'
      it_dd42v     = lt_dd42v.
...
FORM update_05. " TTYD CVIS_VEND_CT_REL_KEY_T ( EN , source: E4A )
  DATA: ls_dd42v TYPE dd42v, lt_dd42v TYPE TABLE OF dd42v.

  CLEAR lt_dd42v.
  CALL METHOD lcl_wb=>create_table_type     "#EC *
    EXPORTING
      i_typename   = 'CVIS_VEND_CT_REL_KEY_T'
      i_devclass   = 'CVI_MAPPING'
      i_langu      = 'E'
      i_ddtext     = 'Key Fields Assignment Vendor Contact Person - BP Rel'
      i_rowtype    = 'CVIS_VEND_CT_REL_KEY'
      i_accessmode = 'T'
      i_keydef     = 'D'
      i_keykind    = 'N'
      it_dd42v     = lt_dd42v.
...
FORM update.
  ...
  APPEND 'CVIS_VEND_CT_REL_KEY'                   TO gt_tabname.   "#EC NOTEXT
  APPEND 'CVI_VEND_CT_LINK'                       TO gt_tabname.   "#EC NOTEXT
  APPEND 'CVIS_VEND_CT_LINK_T'                    TO gt_ttypname.  "#EC NOTEXT
  APPEND 'CVIS_VEND_CT_REL_KEY_T'                 TO gt_ttypname.  "#EC NOTEXT

  PERFORM update_01. " TABD CVIS_VEND_CT_REL_KEY ( EN , source: E4A )
  PERFORM update_02. " TABD CVI_VEND_CT_LINK ( EN , source: E4A )
  PERFORM update_03. " TABT CVI_VEND_CT_LINK ( EN , source: E4A )
  PERFORM update_04. " TTYD CVIS_VEND_CT_LINK_T ( EN , source: E4A )
  PERFORM update_05. " TTYD CVIS_VEND_CT_REL_KEY_T ( EN , source: E4A )
