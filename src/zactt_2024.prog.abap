...
*//
*//© 2024.  For more information, contact Deloitte Touche Tohmatsu Limited.  All rights reserved.
*//
*//====================================================================
...
*----------------------------------------------------------------------*
*21.0  04/24/2024    Piyush Mittal   Added objectid column while joining CDHDR & CDPOS tables
*21.0  02/20/2024    Aditya Sai G    New tables added for extraction (TVKVT, TVKDT, T184)
*21.0  02/20/2024    Aditya Sai G    Addition of new columns to the TVAK table (LFARV, FKARV, FKARA)
*21.0  02/19/2024    Alekhya M       New tables added for extraction (RSZCOMPIC,RSZELTDIR,RSZELTTXT)
*21.0  01/05/2024    Alekhya M       Extended the data type length of column SECURITY_POLICY for table USR02_CC
*20.0  05/11/2023    Gautam Jha      New table added for extraction (USRACL)
*20.0  05/11/2023    Gautam Jha      New table added for extraction for BW/BWHANA-specific (RSZCOMPDIR)
*20.0  05/11/2023    Gautam Jha      New tables added for extraction for GRC-specific (GRACEXCLUDEDOBJS, GRACFFLOG)
*20.0  05/11/2023    Gautam Jha      Removed the extraction of tables USH04 and USR04
*20.0  05/11/2023    Gautam Jha      Extended the data type length of columns VTEXT and PSTYV for table TVAPT
*19.1  02/07/2023    Gautam Jha      Removed the filter set on column TRCLI for TPALOG table and renaming it to TPALOG_ALL
*19.1  02/07/2023    Gautam Jha      Added the extraction of table TB003T (BP Roles)
*19.1  02/07/2023    Gautam Jha      Added a filter on user id (SAP*, DDIC, SAPCPIC, TMSADM, EARLYWATCH) while extracting information related to password status
*19.0	 04/20/2022    Piyush Mittal   Added extraction of SM51
*19.0	 04/18/2022    Ramya Sindhu G  Removed Filters for DD09L table.
*19.0	 04/14/2022    Ramya Sindhu G  Added new tables (TACTT,T001K,/TMWFLOW/PROJCTC,/TMWFLOW/PROJCTT)
*19.0	 03/15/2022    Ramya Sindhu G  Added a new parameter to the ACTT Config Settings for the ACTT Intelligent Rule Template Suggestions feature
*19.0	 01/06/2022    Ramya Sindhu G  Removed column SEX from the ADRP table
*18.0  02/25/2021    Aditya Sai G    New Tables (TVUVF,TVUVS,T049L,REGUH,T042I,T014,AUSP,T16FH,T16FD,T16FW,T078K,T079M,TVKWZ,T025T,TJ30,TJ30T,CABNT,CRMC_PARTNER_FT)
*18.0  02/25/2021    Aditya Sai G    New Tables (TSOCM_BPFCT_DEF2,TSOCM_COND_MAPP,TKES1,TKES2,/TMWFLOW/IMPSTAN,T042Z,T042ZT,T685T,T683T,T169L,T043,T683U)
*18.0  02/25/2021    Aditya Sai G    Added columns to TVAK.
*18.0  02/25/2021    Aditya Sai G    Removed the filter on message numbers on table T100.
*18.0  02/25/2021    Aditya Sai G    Removed the filter for custom transactions on TSTCT table.
*18.0  02/25/2021    Aditya Sai G    Additional filters/columns added for SM20 logs extraction.
*18.0  02/25/2021    Aditya Sai G    Added a check for T685 & T682I existence in the system before extraction.
...
*Added new table RSZCOMPDIR(BW/BWHANA Specific) (Release 20.0)
DATA: ISTABLE_RSZCOMPDIR TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'RSZCOMPDIR' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_RSZCOMPDIR = 'Yes'.
ELSE.
  ISTABLE_RSZCOMPDIR = 'No'.
ENDIF.

IF ISTABLE_RSZCOMPDIR = 'Yes'.
TYPES: BEGIN OF TS_RSZCOMPDIR,
COMPUID(25) Type C,
OBJVERS(1) Type C,
COMPID(30) Type C,
VERSION(10) Type I,
...
LASTUSED(15) TYPE P,
CREATED(15) TYPE P,
CHANGED_WITH(1) Type C,
INFOAREA(30) Type C,
END OF TS_RSZCOMPDIR.
DATA: IT_RSZCOMPDIR TYPE STANDARD TABLE OF TS_RSZCOMPDIR .
DATA: WA_RSZCOMPDIR TYPE TS_RSZCOMPDIR .
DATA:  LV_RSZCOMPDIR TYPE STRING.

*Added new table RSZCOMPIC(BW/BWHANA Specific) (Release 21)

DATA: ISTABLE_RSZCOMPIC TYPE STRING.
SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'RSZCOMPIC' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_RSZCOMPIC = 'Yes'.
ELSE.
  ISTABLE_RSZCOMPIC = 'No'.
ENDIF.

IF ISTABLE_RSZCOMPIC = 'Yes'.
Types: Begin of TS_RSZCOMPIC,
COMPUID(25) TYPE C,
OBJVERS(1) TYPE C,
INFOCUBE(30) TYPE C,
End of TS_RSZCOMPIC.
Data: IT_RSZCOMPIC TYPE STANDARD TABLE OF TS_RSZCOMPIC .
Data: WA_RSZCOMPIC TYPE TS_RSZCOMPIC .
data:  LV_RSZCOMPIC type string.
ENDIF.

ENDIF.
*Added new table RSZELTTXT(BW/BWHANA Specific) (Release 21)

DATA: ISTABLE_RSZELTTXT TYPE STRING.
SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'RSZELTTXT' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_RSZELTTXT = 'Yes'.
ELSE.
  ISTABLE_RSZELTTXT = 'No'.
ENDIF.

IF ISTABLE_RSZELTTXT = 'Yes'.
Types: Begin of TS_RSZELTTXT,
ELTUID(25) Type C,
OBJVERS(1) Type C,
TXTSH(20) Type C,
TXTLG(60) Type C,
End of TS_RSZELTTXT.
Data: IT_RSZELTTXT TYPE STANDARD TABLE OF TS_RSZELTTXT .
Data: WA_RSZELTTXT TYPE TS_RSZELTTXT .
data:  LV_RSZELTTXT type string.
ENDIF.


*Added new table RSZELTDIR(BW/BWHANA Specific) (Release 21)

DATA: ISTABLE_RSZELTDIR TYPE STRING.
SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'RSZELTDIR' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_RSZELTDIR = 'Yes'.
ELSE.
  ISTABLE_RSZELTDIR = 'No'.
ENDIF.

IF ISTABLE_RSZELTDIR = 'Yes'.
Types: Begin of TS_RSZELTDIR,
ELTUID(25) Type C,
OBJVERS(1) Type C,
DEFTP(3) Type C,
End of TS_RSZELTDIR.
Data: IT_RSZELTDIR TYPE STANDARD TABLE OF TS_RSZELTDIR .
Data: WA_RSZELTDIR TYPE TS_RSZELTDIR .
data:  LV_RSZELTDIR type string.
ENDIF.
...
*Added Extraction of Table USRACL (Release 20.0)*
DATA: ISTABLE_USRACL TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'USRACL' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_USRACL = 'Yes'.
ELSE.
...
IF ISTABLE_USRACL = 'Yes'.
  TYPES: BEGIN OF TS_USRACL,
  MANDT(3) TYPE C,
  BNAME(12) TYPE C,
  SNC_SID(8) TYPE C,
  GUIFLAG(1) TYPE C,
  ...
  MTIME(6) TYPE T,
  END OF TS_USRACL.
  DATA: IT_USRACL TYPE STANDARD TABLE OF TS_USRACL .
  DATA: WA_USRACL TYPE TS_USRACL .
  DATA:  LV_USRACL TYPE STRING.
ENDIF.
...
* *Commented the code to avoid the extraction (Release 20.0)
...
ENDIF.
DATA: ISTABLE_TJ30 TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TJ30' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TJ30 = 'Yes'.
ELSE.
...
End of TS_TJ30.
Data: IT_TJ30 TYPE STANDARD TABLE OF TS_TJ30 .
Data: WA_TJ30 TYPE TS_TJ30 .
data:  LV_TJ30 type string.
ENDIF.

DATA: ISTABLE_TJ30T TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TJ30T' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TJ30T = 'Yes'.
ELSE.
...
IF ISTABLE_TJ30T = 'Yes'.
Types: Begin of TS_TJ30T,
MANDT(3) TYPE C,
STSMA(8) TYPE C,
ESTAT(5) TYPE C,
SPRAS(1) TYPE C,
...
End of TS_TJ30T.
Data: IT_TJ30T TYPE STANDARD TABLE OF TS_TJ30T .
Data: WA_TJ30T TYPE TS_TJ30T .
data:  LV_TJ30T type string.
ENDIF.

DATA: ISTABLE_TSOCM_BPFCT_DEF2 TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TSOCM_BPFCT_DEF2' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TSOCM_BPFCT_DEF2 = 'Yes'.
ELSE.
  ISTABLE_TSOCM_BPFCT_DEF2 = 'No'.
ENDIF.

IF ISTABLE_TSOCM_BPFCT_DEF2 = 'Yes'.
Types: Begin of TS_TSOCM_BPFCT_DEF2,
CLIENT(3) TYPE C,
PROCESS_TYPE_F(4) TYPE C,
USER_STATUS(5) TYPE C,
PARTNER_FCT_F(8) TYPE C,
PARTNER_FCT_COPY(8) TYPE C,
PARTNER_FCT_CHEC(8) TYPE C,

End of TS_TSOCM_BPFCT_DEF2.
Data: IT_TSOCM_BPFCT_DEF2 TYPE STANDARD TABLE OF TS_TSOCM_BPFCT_DEF2 .
Data: WA_TSOCM_BPFCT_DEF2 TYPE TS_TSOCM_BPFCT_DEF2 .
data:  LV_TSOCM_BPFCT_DEF2 type string.
ENDIF.

DATA: ISTABLE_TSOCM_COND_MAPP TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TSOCM_COND_MAPP' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TSOCM_COND_MAPP = 'Yes'.
ELSE.
  ISTABLE_TSOCM_COND_MAPP = 'No'.
ENDIF.

IF ISTABLE_TSOCM_COND_MAPP = 'Yes'.
Types: Begin of TS_TSOCM_COND_MAPP,
MANDT(3) TYPE C,
PROC_TYPE(4) TYPE C,
STSMA(8) TYPE C,
USER_STATUS(5) TYPE C,
CHECK_ORDER(2) TYPE N,
CONDITION_ID(20) TYPE C,
ARBGB(20) TYPE C,
MSGNR(3) TYPE C,
TYPE(1) TYPE C,
STATUS_RESET(5) TYPE C,
RESET_APPR(1) TYPE C,

End of TS_TSOCM_COND_MAPP.
Data: IT_TSOCM_COND_MAPP TYPE STANDARD TABLE OF TS_TSOCM_COND_MAPP .
Data: WA_TSOCM_COND_MAPP TYPE TS_TSOCM_COND_MAPP .
data:  LV_TSOCM_COND_MAPP type string.
ENDIF.

DATA: ISTABLE_CRMC_PARTNER_FT TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'CRMC_PARTNER_FT' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_CRMC_PARTNER_FT = 'Yes'.
ELSE.
  ISTABLE_CRMC_PARTNER_FT = 'No'.
ENDIF.
IF ISTABLE_CRMC_PARTNER_FT = 'Yes'.
Types: Begin of TS_CRMC_PARTNER_FT,
CLIENT(3) TYPE C,
PARTNER_FCT(8) Type C,
SPRAS(1) TYPE C,
DESCRIPTION(30) TYPE C,

End of TS_CRMC_PARTNER_FT.
Data: IT_CRMC_PARTNER_FT TYPE STANDARD TABLE OF TS_CRMC_PARTNER_FT .
Data: WA_CRMC_PARTNER_FT TYPE TS_CRMC_PARTNER_FT .
data:  LV_CRMC_PARTNER_FT type string.
ENDIF.

DATA: ISTABLE_T012K TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T012K' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T012K = 'Yes'.
ELSE.
  ISTABLE_T012K = 'No'.
ENDIF.
IF ISTABLE_T012K = 'Yes'.
Types: Begin of TS_T012K,
MANDT(3) TYPE C,
BUKRS(4) TYPE C,
HBKID(5) TYPE C,
HKTID(5) TYPE C,
...
End of TS_T012K.
Data: IT_T012K TYPE STANDARD TABLE OF TS_T012K .
Data: WA_T012K TYPE TS_T012K .
data:  LV_T012K type string.
ENDIF.

DATA: ISTABLE_KNB1 TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'KNB1' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_KNB1 = 'Yes'.
ELSE.
...
End of TS_KNB1.
Data: IT_KNB1 TYPE STANDARD TABLE OF TS_KNB1 .
Data: WA_KNB1 TYPE TS_KNB1 .
data:  LV_KNB1 type string.
ENDIF.

DATA: ISTABLE_T042Z TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T042Z' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T042Z = 'Yes'.
ELSE.
  ISTABLE_T042Z = 'No'.
ENDIF.
IF ISTABLE_T042Z = 'Yes'.
Types: Begin of TS_T042Z,
MANDT(3) TYPE C,
LAND1(3) Type C,
ZLSCH(1) TYPE C,
TEXT1(30) TYPE C,
...
End of TS_T042Z.
Data: IT_T042Z TYPE STANDARD TABLE OF TS_T042Z .
Data: WA_T042Z TYPE TS_T042Z .
data:  LV_T042Z type string.
ENDIF.

DATA: ISTABLE_T042ZT TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T042ZT' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T042ZT = 'Yes'.
ELSE.
  ISTABLE_T042ZT = 'No'.
ENDIF.
IF ISTABLE_T042ZT = 'Yes'.
Types: Begin of TS_T042ZT,
MANDT(3) TYPE C,
SPRAS(1) Type C,
LAND1(3) TYPE C,
ZLSCH(1) Type C,
...
End of TS_T042ZT.
Data: IT_T042ZT TYPE STANDARD TABLE OF TS_T042ZT .
Data: WA_T042ZT TYPE TS_T042ZT .
data:  LV_T042ZT type string.
ENDIF.

DATA: ISTABLE_T16FT TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T16FT' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T16FT = 'Yes'.
ELSE.
  ISTABLE_T16FT = 'No'.
ENDIF.
IF ISTABLE_T16FT = 'Yes'.
Types: Begin of TS_T16FT,
MANDT(3) TYPE C,
SPRAS(1) Type C,
FRGGR(2) TYPE C,
FRGSX(2) Type C,
...
End of TS_T16FT.
Data: IT_T16FT TYPE STANDARD TABLE OF TS_T16FT .
Data: WA_T16FT TYPE TS_T16FT .
data:  LV_T16FT type string.
ENDIF.

DATA: ISTABLE_/TMWFLOW/IMPSTAN TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = '/TMWFLOW/IMPSTAN' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_/TMWFLOW/IMPSTAN = 'Yes'.
ELSE.
  ISTABLE_/TMWFLOW/IMPSTAN = 'No'.
ENDIF.
IF ISTABLE_/TMWFLOW/IMPSTAN = 'Yes'.
Types: Begin of TS_/TMWFLOW/IMPSTAN,
SUB_LANDSCAPE_ID(110) Type C,
BRANCH_NAME(30) Type C,
CYCLE_TYPE(1) Type C,
SYS_NAME(10) Type C,
SYS_TYPE(16) Type C,
SYS_CLIENT(3) Type C,
...
End of TS_/TMWFLOW/IMPSTAN.
Data: IT_/TMWFLOW/IMPSTAN TYPE STANDARD TABLE OF TS_/TMWFLOW/IMPSTAN .
Data: WA_/TMWFLOW/IMPSTAN TYPE TS_/TMWFLOW/IMPSTAN .
data:  LV_/TMWFLOW/IMPSTAN type string.
ENDIF.

DATA: ISTABLE_/TMWFLOW/PROJCTC TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = '/TMWFLOW/PROJCTC' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_/TMWFLOW/PROJCTC = 'Yes'.
ELSE.
  ISTABLE_/TMWFLOW/PROJCTC = 'No'.
ENDIF.
IF ISTABLE_/TMWFLOW/PROJCTC = 'Yes'.
Types: Begin of TS_/TMWFLOW/PROJCTC,
TASKLIST(10) Type C,
PROJECT_TYPE(1) Type C,
SMI_PROJECT(10) Type C,
PARENT_TASKLIST(10) Type C,
RESP_USER(12) Type C,
CREATED_DATE(8) Type D,
STATUS(4) Type C,
ORIGINATOR_KEY(32) Type C,

End of TS_/TMWFLOW/PROJCTC.
Data: IT_/TMWFLOW/PROJCTC TYPE STANDARD TABLE OF TS_/TMWFLOW/PROJCTC .
Data: WA_/TMWFLOW/PROJCTC TYPE TS_/TMWFLOW/PROJCTC .
data:  LV_/TMWFLOW/PROJCTC type string.
ENDIF.

DATA: ISTABLE_/TMWFLOW/PROJCTT TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = '/TMWFLOW/PROJCTT' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_/TMWFLOW/PROJCTT = 'Yes'.
ELSE.
  ISTABLE_/TMWFLOW/PROJCTT = 'No'.
ENDIF.
IF ISTABLE_/TMWFLOW/PROJCTT = 'Yes'.
Types: Begin of TS_/TMWFLOW/PROJCTT,
LANGU(1) Type C,
TASKLIST(10) Type C,
DESCRIPTION(60) Type C,

End of TS_/TMWFLOW/PROJCTT.
Data: IT_/TMWFLOW/PROJCTT TYPE STANDARD TABLE OF TS_/TMWFLOW/PROJCTT .
Data: WA_/TMWFLOW/PROJCTT TYPE TS_/TMWFLOW/PROJCTT .
data:  LV_/TMWFLOW/PROJCTT type string.
ENDIF.

DATA: ISTABLE_T043 TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T043' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T043 = 'Yes'.
ELSE.
...
End of TS_T043.
Data: IT_T043 TYPE STANDARD TABLE OF TS_T043 .
Data: WA_T043 TYPE TS_T043 .
data:  LV_T043 type string.
ENDIF.

DATA: ISTABLE_T683U TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T683U' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T683U = 'Yes'.
ELSE.
  ISTABLE_T683U = 'No'.
ENDIF.
IF ISTABLE_T683U = 'Yes'.
Types: Begin of TS_T683U,
MANDT(3) TYPE C,
SPRAS(1) Type C,
KVEWE(1) TYPE C,
KAPPL(2) TYPE C,
...
End of TS_T683U.
Data: IT_T683U TYPE STANDARD TABLE OF TS_T683U .
Data: WA_T683U TYPE TS_T683U .
data:  LV_T683U type string.
ENDIF.

DATA: ISTABLE_T685T TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T685T' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T685T = 'Yes'.
ELSE.
  ISTABLE_T685T = 'No'.
ENDIF.
IF ISTABLE_T685T = 'Yes'.
Types: Begin of TS_T685T,
MANDT(3) TYPE C,
SPRAS(1) Type C,
KVEWE(1) TYPE C,
KAPPL(2) TYPE C,
...
End of TS_T685T.
Data: IT_T685T TYPE STANDARD TABLE OF TS_T685T .
Data: WA_T685T TYPE TS_T685T .
data:  LV_T685T type string.
ENDIF.

DATA: ISTABLE_CABNT TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'CABNT' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_CABNT = 'Yes'.
ELSE.
  ISTABLE_CABNT = 'No'.
ENDIF.
IF ISTABLE_CABNT = 'Yes'.
Types: Begin of TS_CABNT,
MANDT(3) TYPE C,
ATINN(10) Type C,
SPRAS(1) TYPE C,
ADZHL(4) TYPE C,
...
End of TS_CABNT.
Data: IT_CABNT TYPE STANDARD TABLE OF TS_CABNT .
Data: WA_CABNT TYPE TS_CABNT .
data:  LV_CABNT type string.
ENDIF.

*Added new table GRACEXCLUDEDOBJS (GRC-Specific) (Release 20.0)
DATA: ISTABLE_GRACEXCLUDEDOBJS TYPE STRING.
SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'GRACEXCLUDEDOBJS' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_GRACEXCLUDEDOBJS = 'Yes'.
ELSE.
  ISTABLE_GRACEXCLUDEDOBJS = 'No'.
ENDIF.

IF ISTABLE_GRACEXCLUDEDOBJS = 'Yes'.
Types: Begin of TS_GRACEXCLUDEDOBJS,
MANDT(3) TYPE C,
OBJECTTYPE(3) TYPE I,
FROMVAL(50) TYPE C,
TOVAL(50) TYPE C,
...
ACTIVE(1) TYPE C,
COMMENTS(150) TYPE C,
End of TS_GRACEXCLUDEDOBJS.
Data: IT_GRACEXCLUDEDOBJS TYPE STANDARD TABLE OF TS_GRACEXCLUDEDOBJS .
Data: WA_GRACEXCLUDEDOBJS TYPE TS_GRACEXCLUDEDOBJS .
data:  LV_GRACEXCLUDEDOBJS type string.
ENDIF.
...
*Added new table GRACFFLOG (GRC-Specific) (Release 20.0)
DATA: ISTABLE_GRACFFLOG TYPE STRING.
SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'GRACFFLOG' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_GRACFFLOG = 'Yes'.
ELSE.
  ISTABLE_GRACFFLOG = 'No'.
ENDIF.

IF ISTABLE_GRACFFLOG = 'Yes'.
Types: Begin of TS_GRACFFLOG,
MANDT(3) TYPE C,
FFLOG_ID(32) TYPE C,
FFOBJECT(30) TYPE C,
CONNECTOR(32) TYPE C,
FF_USER(12) TYPE C,
LOGON_TIME(15) TYPE P,
FF_OWNER(12) TYPE C,
WORKFLOW_SENT(1) TYPE C,
REASON_TEXT(40) TYPE C,
UPDATED_AT(15) TYPE P,
LOG_FROM_PLUGIN(1) TYPE C,
LOGOFF_TIME(15) TYPE P,
LOGIN_STATUS(1) TYPE C,
End of TS_GRACFFLOG.
Data: IT_GRACFFLOG TYPE STANDARD TABLE OF TS_GRACFFLOG .
Data: WA_GRACFFLOG TYPE TS_GRACFFLOG .
data:  LV_GRACFFLOG type string.
ENDIF.
...
DATA: ISTABLE_AUSP TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'AUSP' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_AUSP = 'Yes'.
ELSE.
...
ATVGLART(1) Type C,
END OF TS_AUSP.
DATA: IT_AUSP TYPE STANDARD TABLE OF TS_AUSP .
DATA: WA_AUSP TYPE TS_AUSP .
DATA:  LV_AUSP TYPE STRING.

ENDIF.


DATA: ISTABLE_TVUVF TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TVUVF' AND TABCLASS <> 'VIEW'.
...
IF ISTABLE_TVUVF = 'Yes'.
  TYPES: BEGIN OF TS_TVUVF,
  MANDT(3) TYPE C,
  FEHGR(2) TYPE C,
  TBNAM(30) TYPE C,
  FDNAM(30) TYPE C,
  ...
  SORTF(4) TYPE C,
  END OF TS_TVUVF.
  DATA: IT_TVUVF TYPE STANDARD TABLE OF TS_TVUVF .
  DATA: WA_TVUVF TYPE TS_TVUVF .
  DATA:  LV_TVUVF TYPE STRING.
ENDIF.

DATA: ISTABLE_TVUVS TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TVUVS' AND TABCLASS <> 'VIEW'.
...
IF ISTABLE_TVUVS = 'Yes'.
  TYPES: BEGIN OF TS_TVUVS,
  MANDT(3) TYPE C,
  STATG(2) TYPE C,
  UVALL(1) TYPE C,
  UVVLK(1) TYPE C,
  ...
  UVGEK(1) TYPE C,
  END OF TS_TVUVS.
  DATA: IT_TVUVS TYPE STANDARD TABLE OF TS_TVUVS .
  DATA: WA_TVUVS TYPE TS_TVUVS .
  DATA:  LV_TVUVS TYPE STRING.
ENDIF.

DATA: ISTABLE_T049L TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T049L' AND TABCLASS <> 'VIEW'.
...
IF ISTABLE_T049L = 'Yes'.
  TYPES: BEGIN OF TS_T049L,
  MANDT(3) TYPE C,
  BUKRS(4) TYPE C,
  LOCKB(7) TYPE C,
  HBKID(5) TYPE C,
  ...
  ADRNR(10) TYPE C,
  END OF TS_T049L.
  DATA: IT_T049L TYPE STANDARD TABLE OF TS_T049L .
  DATA: WA_T049L TYPE TS_T049L .
  DATA:  LV_T049L TYPE STRING.
ENDIF.

DATA: ISTABLE_T042I TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T042I' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T042I = 'Yes'.
ELSE.
...
IF ISTABLE_T042I = 'Yes'.
TYPES: BEGIN OF TS_T042I,
MANDT(3) Type C,
ZBUKR(4) Type C,
HBKID(5) Type C,
ZLSCH(1) Type C,
...
GSBER(4) Type C,
END OF TS_T042I.
DATA: IT_T042I TYPE STANDARD TABLE OF TS_T042I .
DATA: WA_T042I TYPE TS_T042I .
DATA:  LV_T042I TYPE STRING.

ENDIF.

DATA: ISTABLE_T16FW TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T16FW' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T16FW = 'Yes'.
ELSE.
...
IF ISTABLE_T16FW = 'Yes'.
TYPES: BEGIN OF TS_T16FW,
MANDT(3) Type C,
FRGGR(2) Type C,
FRGCO(2) Type C,
WERKS(4) Type C,
...
OBJID(12) Type C,
END OF TS_T16FW.
DATA: IT_T16FW TYPE STANDARD TABLE OF TS_T16FW .
DATA: WA_T16FW TYPE TS_T16FW .
DATA:  LV_T16FW TYPE STRING.

ENDIF.


DATA: ISTABLE_T16FD TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T16FD' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T16FD = 'Yes'.
ELSE.
...
IF ISTABLE_T16FD = 'Yes'.
TYPES: BEGIN OF TS_T16FD,
MANDT(3) Type C,
SPRAS(1) Type C,
FRGGR(2) Type C,
FRGCO(2) Type C,
FRGCT(20) Type C,
END OF TS_T16FD.
DATA: IT_T16FD TYPE STANDARD TABLE OF TS_T16FD .
DATA: WA_T16FD TYPE TS_T16FD .
DATA:  LV_T16FD TYPE STRING.

ENDIF.


DATA: ISTABLE_T16FH TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T16FH' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T16FH = 'Yes'.
ELSE.
...
IF ISTABLE_T16FH = 'Yes'.
TYPES: BEGIN OF TS_T16FH,
MANDT(3) Type C,
SPRAS(1) Type C,
FRGGR(2) Type C,
FRGGT(20) Type C,
END OF TS_T16FH.
DATA: IT_T16FH TYPE STANDARD TABLE OF TS_T16FH .
DATA: WA_T16FH TYPE TS_T16FH .
DATA:  LV_T16FH TYPE STRING.

ENDIF.

DATA: ISTABLE_T169L TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T169L' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T169L = 'Yes'.
ELSE.
...
IF ISTABLE_T169L = 'Yes'.
TYPES: BEGIN OF TS_T169L,
MANDT(3) Type C,
BUKRS(4) Type C,
TOGRR(4) Type C,
TEXT(40) Type C,
...
TRKPP(4) Type P,
END OF TS_T169L.
DATA: IT_T169L TYPE STANDARD TABLE OF TS_T169L .
DATA: WA_T169L TYPE TS_T169L .
DATA:  LV_T169L TYPE STRING.

ENDIF.

DATA: ISTABLE_T025T TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T025T' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T025T = 'Yes'.
ELSE.
...
IF ISTABLE_T025T = 'Yes'.
TYPES: BEGIN OF TS_T025T,
MANDT(3) Type C,
SPRAS(1) Type C,
BKLAS(4) Type C,
BKBEZ(25) Type C,
END OF TS_T025T.
DATA: IT_T025T TYPE STANDARD TABLE OF TS_T025T .
DATA: WA_T025T TYPE TS_T025T .
DATA:  LV_T025T TYPE STRING.

ENDIF.

DATA: ISTABLE_REGUH TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'REGUH' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_REGUH = 'Yes'.
ELSE.
...
UZAWE(2) Type C,
END OF TS_REGUH.
DATA: IT_REGUH TYPE STANDARD TABLE OF TS_REGUH .
DATA: WA_REGUH TYPE TS_REGUH .
DATA:  LV_REGUH TYPE STRING.

ENDIF.

DATA: ISTABLE_T014 TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T014' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T014 = 'Yes'.
ELSE.
...
ALLCC(1) Type C,
END OF TS_T014.
DATA: IT_T014 TYPE STANDARD TABLE OF TS_T014 .
DATA: WA_T014 TYPE TS_T014 .
DATA:  LV_T014 TYPE STRING.

ENDIF.

DATA: ISTABLE_T078K TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T078K' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T078K = 'Yes'.
ELSE.
...
IF ISTABLE_T078K = 'Yes'.
TYPES: BEGIN OF TS_T078K,
MANDT(3) Type C,
TCODE(20) Type C,
FAUSA(40) Type C,
FAUSF(40) Type C,
...
FAUS2(40) Type C,
END OF TS_T078K.
DATA: IT_T078K TYPE STANDARD TABLE OF TS_T078K .
DATA: WA_T078K TYPE TS_T078K .
DATA:  LV_T078K TYPE STRING.

ENDIF.

DATA: ISTABLE_T079M TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T079M' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T079M = 'Yes'.
ELSE.
...
IF ISTABLE_T079M = 'Yes'.
TYPES: BEGIN OF TS_T079M,
MANDT(3) Type C,
EKORG(4) Type C,
FAUSM(40) Type C,
FAUST(40) Type C,
...
FAUSU(40) Type C,
END OF TS_T079M.
DATA: IT_T079M TYPE STANDARD TABLE OF TS_T079M .
DATA: WA_T079M TYPE TS_T079M .
DATA:  LV_T079M TYPE STRING.

ENDIF.

DATA: ISTABLE_TVKWZ TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TVKWZ' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TVKWZ = 'Yes'.
ELSE.
...
IF ISTABLE_TVKWZ = 'Yes'.
TYPES: BEGIN OF TS_TVKWZ,
MANDT(3) Type C,
VKORG(4) Type C,
VTWEG(2) Type C,
WERKS(4) Type C,
END OF TS_TVKWZ.
DATA: IT_TVKWZ TYPE STANDARD TABLE OF TS_TVKWZ .
DATA: WA_TVKWZ TYPE TS_TVKWZ .
DATA:  LV_TVKWZ TYPE STRING.

ENDIF.

DATA: ISTABLE_TKES1 TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TKES1' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TKES1 = 'Yes'.
ELSE.
...
IF ISTABLE_TKES1 = 'Yes'.
TYPES: BEGIN OF TS_TKES1,
MANDT(3) Type C,
APPLCLASS(4) Type C,
SUBCLASS(2) Type C,
TABNAME(30) Type C,
...
MODTI(6) Type T,
END OF TS_TKES1.
DATA: IT_TKES1 TYPE STANDARD TABLE OF TS_TKES1 .
DATA: WA_TKES1 TYPE TS_TKES1 .
DATA:  LV_TKES1 TYPE STRING.

ENDIF.

DATA: ISTABLE_TKES2 TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TKES2' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TKES2 = 'Yes'.
ELSE.
...
IF ISTABLE_TKES2 = 'Yes'.
TYPES: BEGIN OF TS_TKES2,
MANDT(3) Type C,
SPRAS(1) Type C,
APPLCLASS(4) Type C,
SUBCLASS(2) Type C,
...
TXTLG(40) Type C,
END OF TS_TKES2.
DATA: IT_TKES2 TYPE STANDARD TABLE OF TS_TKES2 .
DATA: WA_TKES2 TYPE TS_TKES2 .
DATA:  LV_TKES2 TYPE STRING.
...
DATA: ISTABLE_TACTT TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TACTT' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TACTT = 'Yes'.
ELSE.
...
IF ISTABLE_TACTT = 'Yes'.
Types: Begin of TS_TACTT,
SPRAS(1) Type C,
ACTVT(2) Type C,
LTEXT(30) Type C,
End of TS_TACTT.
Data: IT_TACTT TYPE STANDARD TABLE OF TS_TACTT .
Data: WA_TACTT TYPE TS_TACTT .
data:  LV_TACTT type string.
ENDIF.

DATA: ISTABLE_T001K TYPE STRING.
SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T001K' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T001K = 'Yes'.
ELSE.
...
IF ISTABLE_T001K = 'Yes'.
Types: Begin of TS_T001K,
MANDT(3) TYPE C,
BWKEY(4) TYPE C,
BUKRS(4) TYPE C,
BWMOD(4) TYPE C,
MLBWA(1) TYPE C,
End of TS_T001K.
Data: IT_T001K TYPE STANDARD TABLE OF TS_T001K .
Data: WA_T001K TYPE TS_T001K .
data:  LV_T001K type string.
ENDIF.
...
DATA: ISTABLE_T100_SM TYPE STRING.
...
IF ISTABLE_T100_SM = 'Yes'.
Types: Begin of TS_T100_SM,
SPRSL(1) Type C,
ARBGB(20) Type C,
MSGNR(3) Type C,
TEXT(73) Type C,
End of TS_T100_SM.
Data: IT_T100_SM TYPE STANDARD TABLE OF TS_T100_SM .
Data: WA_T100_SM TYPE TS_T100_SM .
data:  LV_T100_SM type string.
ENDIF.
...
DATA: ISTABLE_TB003T TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TB003T' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TB003T = 'Yes'.
ELSE.
...
IF ISTABLE_TB003T = 'Yes'.
Types: Begin of TS_TB003T,
CLIENT(3) Type C,
SPRAS(1) Type C,
ROLE(6) Type C,
RLTITL(25) Type C,
RLTXT(50) Type C,
End of TS_TB003T.
Data: IT_TB003T TYPE STANDARD TABLE OF TS_TB003T .
Data: WA_TB003T TYPE TS_TB003T .
data:  LV_TB003T type string.
ENDIF.
...
*ACTTSAP 21.0 Adding Pricing Procedures: Procedure: Texts table - TVKVT *
DATA: ISTABLE_TVKVT TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TVKVT' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TVKVT = 'Yes'.
ELSE.
...
IF ISTABLE_TVKVT = 'Yes'.
Types: Begin of TS_TVKVT,
MANDT(3) Type C,
SPRAS(1) Type C,
KALVG(1) Type C,
VTEXT(20) Type C,
End of TS_TVKVT.
Data: IT_TVKVT TYPE STANDARD TABLE OF TS_TVKVT .
Data: WA_TVKVT TYPE TS_TVKVT .
data:  LV_TVKVT type string.
ENDIF.

*ACTTSAP 21.0 Adding Pricing Procedures: Procedure: Texts table - TVKDT *
DATA: ISTABLE_TVKDT TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'TVKDT' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_TVKDT = 'Yes'.
ELSE.
...
IF ISTABLE_TVKDT = 'Yes'.
Types: Begin of TS_TVKDT,
MANDT(3) Type C,
SPRAS(1) Type C,
KALKS(1) Type C,
VTEXT(20) Type C,
End of TS_TVKDT.
Data: IT_TVKDT TYPE STANDARD TABLE OF TS_TVKDT .
Data: WA_TVKDT TYPE TS_TVKDT .
data:  LV_TVKDT type string.
ENDIF.

*ACTTSAP 21.0 Adding Pricing Procedures: Procedure: Texts table - T184 *
DATA: ISTABLE_T184 TYPE STRING.

SELECT single * FROM DD02L into ls_dd02l
WHERE TABNAME  = 'T184' AND TABCLASS <> 'VIEW'.
.
IF SY-SUBRC = 0.
  ISTABLE_T184 = 'Yes'.
ELSE.
...
ERNAM(12) Type C,
End of TS_T184.
Data: IT_T184 TYPE STANDARD TABLE OF TS_T184 .
Data: WA_T184 TYPE TS_T184 .
data:  LV_T184 type string.
ENDIF.
...
SELECTION-SCREEN ULINE.
SELECTION-SCREEN SKIP 1.
PARAMETERS : ck_sm2 AS CHECKBOX DEFAULT 'X'.
...
INITIALIZATION .
  ...
  comm6 = 'NOTE:'.
  comm7 = '1.If the extraction errors out at SM20 program'.
  comm8 = '  uncheck "Extract SM20 Logs" checkbox and rerun to complete the extraction.'.
  ...
  comm11 = '4.SM20 extraction is performed as a last step of extraction'.
...
*#ACTTSAP12.0 Release 12.0 Changes Begin *
* Hide Exclude Profiles Checkbox*
...
*  Begin of Code for Data Extraction of SM51 report - piyush - 4/20/22

  READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'SM51'.
  IF ck_pop <> 'X' OR sy-subrc <> 0.
* PERFORM extract_sm51.
    DATA : lv_unitALV TYPE dlvunit.
    DATA : lv_ALV TYPE c.
        SELECT SINGLE NAME INTO lv_unitALV
           FROM TRDIR WHERE NAME = 'RSM51000_ALV_NEW'.
       IF sy-subrc = 0.
        lv_ALV = 'X'.
       ENDIF.
      IF lv_ALV <> 'X'.
...
*** End of Change by Srinivas
  ENDIF.
*Addded the extraction of table RSZCOMPDIR (BW/B4HANA specific) (Release 20.0)
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'RSZCOMPDIR'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_RSZCOMPDIR = 'Yes'.
 DATA : lv_unit3 TYPE dlvunit.
 DATA : lv_cv3 TYPE c.
    SELECT SINGLE FIELDNAME INTO lv_unit3
    FROM DD03L WHERE TABNAME = 'RSZCOMPDIR' AND FIELDNAME = 'INFOAREA'.
    IF sy-subrc = 0.
      lv_cv3 = 'X'.
    ENDIF.
   IF lv_cv3 = 'X'.
    DATA: COLUMN_RSZCOMPDIR TYPE STRING.
    DATA TABLE_RSZCOMPDIR TYPE STRING.
    CONCATENATE   'COMPUID OBJVERS COMPID VERSION COMPDIM OBJSTAT'
                  ' CONTREL CONTTIMESTMP OWNER BWAPPL ACTIVFL'
                  ' TIMESTMP TSTPNM TSTPDAT TSTPTIM LASTUSED'
                  ' CREATED CHANGED_WITH INFOAREA'  INTO column_RSZCOMPDIR.

    TABLE_RSZCOMPDIR = 'RSZCOMPDIR'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_RSZCOMPDIR)
   FROM (TABLE_RSZCOMPDIR)
  .
DO.
      "fetch data in chunks for every 'maxsize' number of records
      FETCH NEXT CURSOR s_cursor
      APPENDING TABLE IT_RSZCOMPDIR
      PACKAGE SIZE maxsize.
      IF sy-subrc <> 0.
        CLOSE CURSOR s_cursor.
        EXIT.
      ELSE.
          IF rb_fore IS INITIAL.
            PERFORM SUB_DOWNLOAD_RSZCOMPDIR.
          ENDIF.
      ENDIF.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_RSZCOMPDIR IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_RSZCOMPDIR.
      ENDIF.
    ENDIF.
   ENDIF.
...
*Addded the extraction of table RSZCOMPIC (BW/B4HANA specific) (Release 21)
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'RSZCOMPIC'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

  IF ISTABLE_RSZCOMPIC = 'Yes'.
    DATA: COLUMN_RSZCOMPIC TYPE STRING.
    DATA TABLE_RSZCOMPIC TYPE STRING.
    COLUMN_RSZCOMPIC = 'COMPUID OBJVERS INFOCUBE' .
    TABLE_RSZCOMPIC = 'RSZCOMPIC'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_RSZCOMPIC)
   FROM (TABLE_RSZCOMPIC)
  .
DO.
  "fetch data in chunks for every 'maxsize' number of records
        FETCH NEXT CURSOR s_cursor
        APPENDING TABLE it_RSZCOMPIC
        PACKAGE SIZE maxsize.
        IF sy-subrc <> 0.
          CLOSE CURSOR s_cursor.
          EXIT.
      ...
      .
      .
      lv_file = 'RSZCOMPIC.ACTT'.
      ls_table = 'COMPUID'.
      DESCRIBE FIELD wa_RSZCOMPIC-COMPUID
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'OBJVERS' INTO ls_table.
      DESCRIBE FIELD wa_RSZCOMPIC-OBJVERS
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'INFOCUBE' INTO ls_table.
      DESCRIBE FIELD wa_RSZCOMPIC-INFOCUBE
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      lv_RSZCOMPIC = LINES( it_RSZCOMPIC ) .
      IF sy-subrc = 0.
        PERFORM conv_table_to_text TABLES it_RSZCOMPIC
        USING lv_file
        ls_table .
        FREE it_RSZCOMPIC .
        CLEAR lt_table .
...
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'RSZELTTXT'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

  IF ISTABLE_RSZELTTXT = 'Yes'.
    DATA: COLUMN_RSZELTTXT TYPE STRING.
    DATA TABLE_RSZELTTXT TYPE STRING.
    COLUMN_RSZELTTXT = 'ELTUID OBJVERS TXTSH TXTLG'.
    TABLE_RSZELTTXT = 'RSZELTTXT'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_RSZELTTXT)
   FROM (TABLE_RSZELTTXT)
  .
DO.
  "fetch data in chunks for every 'maxsize' number of records
        FETCH NEXT CURSOR s_cursor
        APPENDING TABLE it_RSZELTTXT
        PACKAGE SIZE maxsize.
        IF sy-subrc <> 0.
          CLOSE CURSOR s_cursor.
          EXIT.
      ...
      .
      .
      lv_file = 'RSZELTTXT.ACTT'.
      ls_table = 'ELTUID'.
      DESCRIBE FIELD wa_RSZELTTXT-ELTUID
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'OBJVERS' INTO ls_table.
      DESCRIBE FIELD wa_RSZELTTXT-OBJVERS
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'TXTSH' INTO ls_table.
      DESCRIBE FIELD wa_RSZELTTXT-TXTSH
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'TXTLG' INTO ls_table.
      DESCRIBE FIELD wa_RSZELTTXT-TXTLG
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      lv_RSZELTTXT = LINES( it_RSZELTTXT ) .
      IF sy-subrc = 0.
        PERFORM conv_table_to_text TABLES it_RSZELTTXT
        USING lv_file
        ls_table .
        FREE it_RSZELTTXT .
        CLEAR lt_table .
...
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'RSZELTDIR'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

  IF ISTABLE_RSZELTDIR = 'Yes'.
    DATA: COLUMN_RSZELTDIR TYPE STRING.
    DATA TABLE_RSZELTDIR TYPE STRING.
    COLUMN_RSZELTDIR = 'ELTUID OBJVERS DEFTP'.
    TABLE_RSZELTDIR = 'RSZELTDIR'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_RSZELTDIR)
   FROM (TABLE_RSZELTDIR)
   where DEFTP = 'REP'.

DO.
  "fetch data in chunks for every 'maxsize' number of records
        FETCH NEXT CURSOR s_cursor
        APPENDING TABLE it_RSZELTDIR
        PACKAGE SIZE maxsize.
        IF sy-subrc <> 0.
          CLOSE CURSOR s_cursor.
          EXIT.
      ...
      .
      .
      lv_file = 'RSZELTDIR.ACTT'.
      ls_table = 'ELTUID'.
      DESCRIBE FIELD wa_RSZELTDIR-ELTUID
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'OBJVERS' INTO ls_table.
      DESCRIBE FIELD wa_RSZELTDIR-OBJVERS
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'DEFTP' INTO ls_table.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
          lv_RSZELTDIR = LINES( it_RSZELTDIR ) .
      IF sy-subrc = 0.
        PERFORM conv_table_to_text TABLES it_RSZELTDIR
        USING lv_file
        ls_table .
        FREE it_RSZELTDIR .
        CLEAR lt_table .
...
.
  ...
  TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
  CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
  LV_TSTCT = lv_TSTCT + LINES( IT_TSTCT ) .
  IF lv_tstct GT 0.
...
*Added Extraction of table USRACL (Release 20.0)*
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'USRACL'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_USRACL = 'Yes'.
    DATA: COLUMN_USRACL TYPE STRING.
    DATA TABLE_USRACL TYPE STRING.
    CONCATENATE   'MANDT BNAME SNC_SID GUIFLAG PNAME HNAME'
                  ' CNAME CDATE CTIME MNAME'
                  ' MDATE MTIME'  INTO column_USRACL.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_USRACL IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_USRACL.
      ENDIF.
    ENDIF.
   ENDIF.
...
*Commented the code to avoid the extraction of USR04 (Release 20.0)
...
*Commented the code to avoid the extraction of USH04 (Release 20.0)
...
* INTO TABLE IT_TPALOG
             FOR ALL ENTRIES IN IT_E070
* Filter applied on column TRCLI has been removed. GAJHA*
             WHERE TRSTEP = 'I' AND TRKORR = IT_E070-TRKORR
             .
* Filter applied on column TRCLI has been removed. GAJHA*
    DO.
      "fetch data in chunks for every 'maxsize' number of records
...
*** End of change by SUDHA
  .
  IF rb_fore IS NOT INITIAL AND IT_DD09L IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_DD09L.
  ENDIF.
ENDIF.
...
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'AUSP'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_AUSP = 'Yes'.
    DATA: COLUMN_AUSP TYPE STRING.
    DATA TABLE_AUSP TYPE STRING.
    CONCATENATE   'MANDT OBJEK ATINN ATZHL MAFID'
                  ' KLART ADZHL ATWRT ATFLV ATFLB ATAW1'
                  ' ATCOD ATTLV ATTLB'
                  ' ATPRZ ATINC ATAUT'
                  ' AENNR DATUV LKENZ ATZIS'  INTO column_AUSP.
...
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_AUSP)
   FROM (TABLE_AUSP)
   WHERE mandt =  s_pclien AND KLART = '032'.
DO.
      "fetch data in chunks for every 'maxsize' number of records
      FETCH NEXT CURSOR s_cursor
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_AUSP IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_AUSP.
      ENDIF.
    ENDIF.
   ENDIF.
...
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TVUVF'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_TVUVF = 'Yes'.
    DATA: COLUMN_TVUVF TYPE STRING.
    DATA TABLE_TVUVF TYPE STRING.
    CONCATENATE   'MANDT FEHGR TBNAM FDNAM STATG FCODE'
                  ' MSGKZ SORTF'  INTO column_TVUVF.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_TVUVF IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_TVUVF.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TVUVS'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_TVUVS = 'Yes'.
    DATA: COLUMN_TVUVS TYPE STRING.
    DATA TABLE_TVUVS TYPE STRING.
    CONCATENATE   'MANDT STATG UVALL UVVLK UVFAK UVPRS'
                  ' UVWAK UVPIK UVPAK UVGEK'  INTO column_TVUVS.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_TVUVS IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_TVUVS.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T049L'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_T049L = 'Yes'.
    DATA: COLUMN_T049L TYPE STRING.
    DATA TABLE_T049L TYPE STRING.
    CONCATENATE   'MANDT BUKRS LOCKB HBKID'
                  ' LCKNR ADRNR'  INTO column_T049L.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_T049L IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_T049L.
      ENDIF.
    ENDIF.
   ENDIF.
...
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T042I'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_T042I = 'Yes'.
    DATA: COLUMN_T042I TYPE STRING.
    DATA TABLE_T042I TYPE STRING.
    CONCATENATE   'MANDT ZBUKR HBKID ZLSCH WAERS HKTID UKONT VKONT'
                  ' GEBKZ GSBER'  INTO column_T042I.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_T042I IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_T042I.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T16FW'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_T16FW = 'Yes'.
    DATA: COLUMN_T16FW TYPE STRING.
    DATA TABLE_T16FW TYPE STRING.
    CONCATENATE   'MANDT FRGGR FRGCO WERKS'
                  ' OTYPE OBJID'  INTO column_T16FW.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_T16FW IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_T16FW.
      ENDIF.
    ENDIF.
   ENDIF.
...
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T16FD'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_T16FD = 'Yes'.
    DATA: COLUMN_T16FD TYPE STRING.
    DATA TABLE_T16FD TYPE STRING.
    CONCATENATE   'MANDT SPRAS FRGGR'
                  ' FRGCO FRGCT'  INTO column_T16FD.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_T16FD IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_T16FD.
      ENDIF.
    ENDIF.
   ENDIF.
...
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T16FH'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_T16FH = 'Yes'.
    DATA: COLUMN_T16FH TYPE STRING.
    DATA TABLE_T16FH TYPE STRING.
    CONCATENATE   'MANDT SPRAS'
                  ' FRGGR FRGGT'  INTO column_T16FH.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_T16FH IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_T16FH.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T169L'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_T169L = 'Yes'.
    DATA: COLUMN_T169L TYPE STRING.
    DATA TABLE_T169L TYPE STRING.
    CONCATENATE   'MANDT BUKRS TOGRR TEXT XAKZPU'
                  ' TAKZPU XAKZPB TAKZPB XAKZPP TAKZPP'
                  ' XAKZNU TAKZNU XAKZNB TAKZNB XAKZNP'
                  ' TAKZNP XRKUER TRKPB XRKPP TRKPP' INTO column_T169L.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_T169L IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_T169L.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T025T'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_T025T = 'Yes'.
    DATA: COLUMN_T025T TYPE STRING.
    DATA TABLE_T025T TYPE STRING.
    CONCATENATE   'MANDT SPRAS BKLAS'
                  ' BKBEZ' INTO column_T025T.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_T025T IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_T025T.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'REGUH'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_REGUH = 'Yes'.
    DATA: COLUMN_REGUH TYPE STRING.
    DATA TABLE_REGUH TYPE STRING.
    CONCATENATE   'MANDT LAUFD LAUFI XVORL ZBUKR LIFNR'
                  ' KUNNR RZAWE UZAWE' INTO column_REGUH.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_REGUH IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_REGUH.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T014'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_T014 = 'Yes'.
    DATA: COLUMN_T014 TYPE STRING.
    DATA TABLE_T014 TYPE STRING.
    CONCATENATE   'MANDT KKBER WAERS STAFO PERIV CTLPC'
                  ' KLIMK SBGRP ALLCC' INTO column_T014.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_T014 IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_T014.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T078K'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_T078K = 'Yes'.
    DATA: COLUMN_T078K TYPE STRING.
    DATA TABLE_T078K TYPE STRING.
    CONCATENATE   'MANDT TCODE FAUSA FAUSF FAUSM FAUS1'
                  ' FAUSW FAUST FAUSG FAUSN'
                  ' FAUSX FAUSU FAUS2' INTO column_T078K.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_T078K IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_T078K.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T079M'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_T079M = 'Yes'.
    DATA: COLUMN_T079M TYPE STRING.
    DATA TABLE_T079M TYPE STRING.
    CONCATENATE   'MANDT EKORG FAUSM'
                  '  FAUST FAUSW FAUSN'
                  ' FAUSX FAUSU' INTO column_T079M.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_T079M IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_T079M.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TVKWZ'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_TVKWZ = 'Yes'.
    DATA: COLUMN_TVKWZ TYPE STRING.
    DATA TABLE_TVKWZ TYPE STRING.
    CONCATENATE   'MANDT VKORG VTWEG'
                  ' WERKS' INTO column_TVKWZ.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_TVKWZ IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_TVKWZ.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TKES1'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_TKES1 = 'Yes'.
    DATA: COLUMN_TKES1 TYPE STRING.
    DATA TABLE_TKES1 TYPE STRING.
    CONCATENATE   'MANDT APPLCLASS SUBCLASS TABNAME FORM GDATE'
                  ' MODCN MODNA MODDA MODTI' INTO column_TKES1.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_TKES1 IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_TKES1.
      ENDIF.
    ENDIF.
   ENDIF.

READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TKES2'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
IF ISTABLE_TKES2 = 'Yes'.
    DATA: COLUMN_TKES2 TYPE STRING.
    DATA TABLE_TKES2 TYPE STRING.
    CONCATENATE   'MANDT SPRAS APPLCLASS SUBCLASS TABNAME FORM'
                  ' TXTKZ TXTMT TXTLG' INTO column_TKES2.
...
.
.
      IF rb_fore IS NOT INITIAL AND IT_TKES2 IS NOT INITIAL .
        PERFORM SUB_DOWNLOAD_TKES2.
      ENDIF.
    ENDIF.
   ENDIF.
...
lv_file = 'T030W.ACTT'.
  ...
  ENDIF.
  ENDIF.
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TACTT'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
  IF ISTABLE_TACTT = 'Yes'.
    DATA: COLUMN_TACTT TYPE STRING.
    DATA TABLE_TACTT TYPE STRING.
    COLUMN_TACTT = 'SPRAS ACTVT LTEXT'.
    TABLE_TACTT = 'TACTT'.

  OPEN CURSOR WITH HOLD s_cursor FOR
SELECT
     (COLUMN_TACTT)
FROM (TABLE_TACTT)  CLIENT SPECIFIED
WHERE SPRAS = 'EN'
.
DO.
      "fetch data in chunks for every 'maxsize' number of records
      FETCH NEXT CURSOR s_cursor
      APPENDING TABLE IT_TACTT
      PACKAGE SIZE maxsize.
      IF sy-subrc <> 0.
        CLOSE CURSOR s_cursor.
        EXIT.
...
lv_file = 'TACTT.ACTT'.
  ls_table = 'SPRAS'.
  DESCRIBE FIELD wa_TACTT-SPRAS
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'ACTVT' INTO ls_table.
  DESCRIBE FIELD wa_TACTT-ACTVT
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'LTEXT' INTO ls_table.
  DESCRIBE FIELD wa_TACTT-LTEXT
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.

  lv_TACTT = LINES( it_TACTT ) .
  IF sy-subrc = 0.
    PERFORM conv_table_to_text TABLES it_TACTT
    USING lv_file
    ls_table .
    FREE IT_TACTT .
    CLEAR lt_table .
...
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T001K'.
  IF ck_pop <> 'X' OR sy-subrc <> 0.
    IF istable_T001K = 'Yes'.
      DATA: column_T001K TYPE string.
      DATA table_T001K TYPE string.
      COLUMN_T001K = 'MANDT BWKEY BUKRS BWMOD MLBWA'.
      TABLE_T001K = 'T001K'.
   OPEN CURSOR WITH HOLD s_cursor FOR
     SELECT
     (COLUMN_T001K)
     FROM (TABLE_T001K)  CLIENT SPECIFIED
     WHERE mandt =  s_pclien
.
DO.
      "fetch data in chunks for every 'maxsize' number of records
      FETCH NEXT CURSOR s_cursor
      APPENDING TABLE IT_T001K
      PACKAGE SIZE maxsize.
      IF sy-subrc <> 0.
        CLOSE CURSOR s_cursor.
        EXIT.
...
.
    lv_file = 'T001K.ACTT'.
    ls_table = 'MANDT'.
    DESCRIBE FIELD WA_T001K-MANDT
    TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
    CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
    CONCATENATE ls_table '|^|' 'BWKEY' into ls_table.
    DESCRIBE FIELD WA_T001K-BWKEY
    TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
    CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
    CONCATENATE ls_table '|^|' 'BUKRS' into ls_table.
    DESCRIBE FIELD WA_T001K-BUKRS
    TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
    CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
    CONCATENATE ls_table '|^|' 'BWMOD' into ls_table.
    DESCRIBE FIELD WA_T001K-BWMOD
    TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
    CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
    CONCATENATE ls_table '|^|' 'MLBWA' into ls_table.
    DESCRIBE FIELD WA_T001K-MLBWA
    TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
    CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
    lv_T001K = lv_T001K + LINES( it_T001K ) .
  IF lv_T001K GT 0.
    perform conv_table_to_text tables it_T001K
    using lv_file
    ls_table .
    FREE IT_T001K.
    CLEAR lt_table .
...
lv_file = 'T096T.ACTT'.
  ...
  DATA : lv_unit1 TYPE dlvunit.
  DATA : lv_cv1 TYPE c.
  SELECT SINGLE NAME INTO lv_unit1
         FROM prdvers WHERE NAME = 'SAP SOLUTION MANAGER' AND INSTSTATUS = '+'.
  IF sy-subrc = 0.
    lv_cv1 = 'X'.
  ENDIF.
IF lv_cv1 = 'X'.
  READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T100_SM'.
  IF ck_pop <> 'X' OR sy-subrc <> 0.
  IF ISTABLE_T100_SM = 'Yes'.
    DATA: COLUMN_T100_SM TYPE STRING.
    DATA TABLE_T100_SM TYPE STRING.
    COLUMN_T100_SM = 'SPRSL ARBGB MSGNR TEXT'.
    TABLE_T100_SM = 'T100'.
...
.
.
lv_file = 'T100_SM.ACTT'.
  ls_table = 'SPRSL'.
  DESCRIBE FIELD wa_t100_sm-SPRSL
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'ARBGB' INTO ls_table.
  DESCRIBE FIELD wa_t100_sm-ARBGB
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'MSGNR' INTO ls_table.
  DESCRIBE FIELD wa_t100_sm-MSGNR
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'TEXT' INTO ls_table.
  DESCRIBE FIELD wa_t100_sm-TEXT
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  lv_t100_sm = lv_t100_sm + LINES( it_t100_sm ) .
  IF lv_t100_sm GT 0.
    PERFORM conv_table_to_text TABLES it_t100_sm
    USING lv_file
    ls_table .
    FREE IT_T100_SM .
    CLEAR lt_table .
...
SELECT
     (COLUMN_T100)
FROM (TABLE_T100)
WHERE ( MSGNR IN ('207','117','108','081','082','083','504') ) AND
 SPRSL = 'EN'.
.
DO.
      "fetch data in chunks for every 'maxsize' number of records
...
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TB003T'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
  IF ISTABLE_TB003T = 'Yes'.
    DATA: COLUMN_TB003T TYPE STRING.
    DATA TABLE_TB003T TYPE STRING.
    COLUMN_TB003T = 'CLIENT SPRAS ROLE RLTITL RLTXT'.
    TABLE_TB003T = 'TB003T'.
...
SELECT
     (COLUMN_TB003T)
FROM (TABLE_TB003T)
WHERE CLIENT = s_pclien AND SPRAS = 'EN'
.
DO.
      "fetch data in chunks for every 'maxsize' number of records
      FETCH NEXT CURSOR s_cursor
      APPENDING TABLE IT_TB003T
      PACKAGE SIZE maxsize.
      IF sy-subrc <> 0.
        CLOSE CURSOR s_cursor.
        EXIT.
...
.
 lv_file = 'TB003T.ACTT'.
  ls_table = 'CLIENT'.
  DESCRIBE FIELD wa_TB003T-CLIENT
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'SPRAS' INTO ls_table.
  DESCRIBE FIELD wa_TB003T-SPRAS
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'ROLE' INTO ls_table.
  DESCRIBE FIELD wa_TB003T-ROLE
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'RLTITL' INTO ls_table.
  DESCRIBE FIELD wa_TB003T-RLTITL
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'RLTXT' INTO ls_table.
  DESCRIBE FIELD wa_TB003T-RLTXT
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  lv_TB003T = LINES( it_TB003T ) .
  IF sy-subrc = 0.
    PERFORM conv_table_to_text TABLES it_TB003T
    USING lv_file
    ls_table .
    FREE IT_TB003T .
    CLEAR lt_table .
...
IF ck_pop <> 'X' OR sy-subrc <> 0.
  IF ISTABLE_TVAK = 'Yes'.
    ...
    COLUMN_TVAK = 'AUART BEZOB CMGRA KLIMP MANDT VBTYP KALVG LFARV FKARV FKARA'.
    TABLE_TVAK = 'TVAK'.
...
lv_file = 'TVAK.ACTT'.
  ...
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'KALVG' INTO ls_table.
  DESCRIBE FIELD wa_tvak-KALVG
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'LFARV' INTO ls_table.
  DESCRIBE FIELD wa_tvak-LFARV
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'FKARV' INTO ls_table.
  DESCRIBE FIELD wa_tvak-FKARV
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'FKARA' INTO ls_table.
  DESCRIBE FIELD wa_tvak-FKARA
  TYPE lv_type LENGTH lv_len IN BYTE MODE.
  CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
...
*TVKVT Table Extraction*
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TVKVT'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
  IF ISTABLE_TVKVT = 'Yes'.
    DATA: COLUMN_TVKVT TYPE STRING.
    DATA TABLE_TVKVT TYPE STRING.
    CONCATENATE  'MANDT SPRAS'
                 ' KALVG VTEXT' INTO COLUMN_TVKVT.
    TABLE_TVKVT = 'TVKVT'.
...
lv_file = 'TVKVT.ACTT'.
ls_table = 'MANDT'.
DESCRIBE FIELD WA_TVKVT-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SPRAS' into ls_table.
DESCRIBE FIELD WA_TVKVT-SPRAS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'KALVG' into ls_table.
DESCRIBE FIELD WA_TVKVT-KALVG
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'VTEXT' into ls_table.
DESCRIBE FIELD WA_TVKVT-VTEXT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
lv_TVKVT = lines( it_TVKVT ) .
IF SY-SUBRC = 0.
perform conv_table_to_text tables it_TVKVT
using lv_file
 ls_table .
FREE IT_TVKVT .
CLEAR lt_table .
...
*TVKDT Table Extraction*
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TVKDT'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
  IF ISTABLE_TVKDT = 'Yes'.
    DATA: COLUMN_TVKDT TYPE STRING.
    DATA TABLE_TVKDT TYPE STRING.
    CONCATENATE  'MANDT SPRAS'
                 ' KALKS VTEXT' INTO COLUMN_TVKDT.
    TABLE_TVKDT = 'TVKDT'.
...
lv_file = 'TVKDT.ACTT'.
ls_table = 'MANDT'.
DESCRIBE FIELD WA_TVKDT-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SPRAS' into ls_table.
DESCRIBE FIELD WA_TVKDT-SPRAS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'KALKS' into ls_table.
DESCRIBE FIELD WA_TVKDT-KALKS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'VTEXT' into ls_table.
DESCRIBE FIELD WA_TVKDT-VTEXT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
lv_TVKDT = lines( it_TVKDT ) .
IF SY-SUBRC = 0.
perform conv_table_to_text tables it_TVKDT
using lv_file
 ls_table .
FREE IT_TVKDT .
CLEAR lt_table .
...
*T184 Table Extraction*
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T184'.
IF ck_pop <> 'X' OR sy-subrc <> 0.
  IF ISTABLE_T184 = 'Yes'.
    DATA: COLUMN_T184 TYPE STRING.
    DATA TABLE_T184 TYPE STRING.
    CONCATENATE  'MANDT AUART MTPOS VWPOS UEPST PSTYV'
                 ' PSTY1 PSTY2 PSTY3 PSTY4 PSTY5 PSTY6'
                 ' PSTY7 PSTY8 PSTY9 PSTY10 PSTY11 ERNAM' INTO COLUMN_T184.
    TABLE_T184 = 'T184'.
...
lv_file = 'T184.ACTT'.
ls_table = 'MANDT'.
DESCRIBE FIELD WA_T184-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'AUART' into ls_table.
DESCRIBE FIELD WA_T184-AUART
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'MTPOS' into ls_table.
DESCRIBE FIELD WA_T184-MTPOS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'VWPOS' into ls_table.
DESCRIBE FIELD WA_T184-VWPOS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UEPST' into ls_table.
DESCRIBE FIELD WA_T184-UEPST
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTYV' into ls_table.
DESCRIBE FIELD WA_T184-PSTYV
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY1' into ls_table.
DESCRIBE FIELD WA_T184-PSTY1
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY2' into ls_table.
DESCRIBE FIELD WA_T184-PSTY2
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY3' into ls_table.
DESCRIBE FIELD WA_T184-PSTY3
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY4' into ls_table.
DESCRIBE FIELD WA_T184-PSTY4
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY5' into ls_table.
DESCRIBE FIELD WA_T184-PSTY5
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY6' into ls_table.
DESCRIBE FIELD WA_T184-PSTY6
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY7' into ls_table.
DESCRIBE FIELD WA_T184-PSTY7
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY8' into ls_table.
DESCRIBE FIELD WA_T184-PSTY8
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY9' into ls_table.
DESCRIBE FIELD WA_T184-PSTY9
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY10' into ls_table.
DESCRIBE FIELD WA_T184-PSTY10
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PSTY11' into ls_table.
DESCRIBE FIELD WA_T184-PSTY11
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ERNAM' into ls_table.
DESCRIBE FIELD WA_T184-ERNAM
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
lv_T184 = lines( it_T184 ) .
IF SY-SUBRC = 0.
perform conv_table_to_text tables it_T184
using lv_file
 ls_table .
FREE IT_T184 .
CLEAR lt_table .
...
IF ck_pop <> 'X' OR sy-subrc <> 0.
    ...
    IF istable_T685 = 'Yes'.
      DATA: column_T685 TYPE string.
      DATA table_T685 TYPE string.
      COLUMN_T685 = 'MANDT KAPPL KSCHL KOZGF'.
      TABLE_T685 = 'T685'.
   OPEN CURSOR WITH HOLD s_cursor FOR
     SELECT
     (COLUMN_T685)
     FROM (TABLE_T685)  CLIENT SPECIFIED
     WHERE mandt =  s_pclien
  .
  DO.
    "fetch data in chunks for every 'maxsize' number of records
...
IF ck_pop <> 'X' OR sy-subrc <> 0.
 IF ISTABLE_T682I = 'Yes'.
    DATA: COLUMN_T682I TYPE STRING.
    DATA TABLE_T682I TYPE STRING.
    COLUMN_T682I = 'KOZGF KOTABNR'.
    TABLE_T682I = 'T682I'.
    SELECT
   (COLUMN_T682I)
    FROM (TABLE_T682I)
...
ENDIF.
*TJ30 Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TJ30'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_TJ30 = 'Yes'.
    DATA: COLUMN_TJ30 TYPE STRING.
    DATA TABLE_TJ30 TYPE STRING.
    CONCATENATE   'MANDT STSMA ESTAT INIST STONR HSONR'
                  ' LINEP STATP BERSL CRM_VRGNG' INTO column_TJ30.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_TJ30.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_TJ30 IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_TJ30.
  ENDIF.
ENDIF.
ENDIF.
*TJ30T Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TJ30T'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_TJ30T = 'Yes'.
    DATA: COLUMN_TJ30T TYPE STRING.
    DATA TABLE_TJ30T TYPE STRING.
    CONCATENATE   'MANDT STSMA ESTAT SPRAS'
                  ' TXT04 TXT30 LTEXT' INTO column_TJ30.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_TJ30T.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_TJ30T IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_TJ30T.
  ENDIF.
ENDIF.
ENDIF.

*TSOCM_BPFCT_DEF2 Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TSOCM_BPFCT_DEF2'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_TSOCM_BPFCT_DEF2 = 'Yes'.
    DATA: COLUMN_TSOCM_BPFCT_DEF2 TYPE STRING.
    DATA TABLE_TSOCM_BPFCT_DEF2 TYPE STRING.
    CONCATENATE   'CLIENT PROCESS_TYPE_F USER_STATUS PARTNER_FCT_F'
                  ' PARTNER_FCT_COPY PARTNER_FCT_CHEC' INTO column_TSOCM_BPFCT_DEF2.

    TABLE_TSOCM_BPFCT_DEF2 = 'TSOCM_BPFCT_DEF2'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_TSOCM_BPFCT_DEF2)
   FROM (TABLE_TSOCM_BPFCT_DEF2)
   WHERE CLIENT =  s_pclien
  .
DO.
  "fetch data in chunks for every 'maxsize' number of records
    FETCH NEXT CURSOR S_CURSOR
    APPENDING TABLE IT_TSOCM_BPFCT_DEF2
    PACKAGE SIZE MAXSIZE.
    IF SY-SUBRC <> 0.
      CLOSE CURSOR S_CURSOR.
      EXIT.
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_TSOCM_BPFCT_DEF2.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_TSOCM_BPFCT_DEF2 IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_TSOCM_BPFCT_DEF2.
  ENDIF.
ENDIF.
ENDIF.

*TSOCM_COND_MAPP Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'TSOCM_COND_MAPP'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_TSOCM_COND_MAPP = 'Yes'.
    DATA: COLUMN_TSOCM_COND_MAPP TYPE STRING.
    DATA TABLE_TSOCM_COND_MAPP TYPE STRING.
    CONCATENATE   'MANDT PROC_TYPE STSMA USER_STATUS CHECK_ORDER CONDITION_ID ARBGB'
                  ' MSGNR TYPE STATUS_RESET RESET_APPR' INTO column_TSOCM_COND_MAPP.

    TABLE_TSOCM_COND_MAPP = 'TSOCM_COND_MAPP'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_TSOCM_COND_MAPP)
   FROM (TABLE_TSOCM_COND_MAPP)
   WHERE MANDT =  s_pclien and CONDITION_ID = 'SAME_USER'
  .
DO.
  "fetch data in chunks for every 'maxsize' number of records
    FETCH NEXT CURSOR S_CURSOR
    APPENDING TABLE IT_TSOCM_COND_MAPP
    PACKAGE SIZE MAXSIZE.
    IF SY-SUBRC <> 0.
      CLOSE CURSOR S_CURSOR.
      EXIT.
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_TSOCM_COND_MAPP.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_TSOCM_COND_MAPP IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_TSOCM_COND_MAPP.
  ENDIF.
ENDIF.
ENDIF.

*CRMC_PARTNER_FT Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'CRMC_PARTNER_FT'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_CRMC_PARTNER_FT = 'Yes'.
    DATA: COLUMN_CRMC_PARTNER_FT TYPE STRING.
    DATA TABLE_CRMC_PARTNER_FT TYPE STRING.
    CONCATENATE   'CLIENT PARTNER_FCT SPRAS'
                  ' DESCRIPTION' INTO column_CRMC_PARTNER_FT.

    TABLE_CRMC_PARTNER_FT = 'CRMC_PARTNER_FT'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_CRMC_PARTNER_FT)
   FROM (TABLE_CRMC_PARTNER_FT)
   WHERE CLIENT =  s_pclien and SPRAS = 'E'
  .
DO.
  "fetch data in chunks for every 'maxsize' number of records
    FETCH NEXT CURSOR S_CURSOR
    APPENDING TABLE IT_CRMC_PARTNER_FT
    PACKAGE SIZE MAXSIZE.
    IF SY-SUBRC <> 0.
      CLOSE CURSOR S_CURSOR.
      EXIT.
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_CRMC_PARTNER_FT.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_CRMC_PARTNER_FT IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_CRMC_PARTNER_FT.
  ENDIF.
ENDIF.
ENDIF.
...
*T012K Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T012K'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_T012K = 'Yes'.
    DATA: COLUMN_T012K TYPE STRING.
    DATA TABLE_T012K TYPE STRING.
    CONCATENATE   'MANDT BUKRS HBKID HKTID BANKN'
                  ' WAERS HKONT' INTO column_T012K.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_T012K.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_T012K IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_T012K.
  ENDIF.
ENDIF.
ENDIF.
...
*KNB1 Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'KNB1'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_KNB1 = 'Yes'.
    DATA: COLUMN_KNB1 TYPE STRING.
    DATA TABLE_KNB1 TYPE STRING.
    CONCATENATE   'MANDT KUNNR BUKRS'
                  ' TOGRU' INTO column_KNB1.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_KNB1.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_KNB1 IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_KNB1.
  ENDIF.
ENDIF.
ENDIF.
...
*T042Z Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T042Z'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_T042Z = 'Yes'.
    DATA: COLUMN_T042Z TYPE STRING.
    DATA TABLE_T042Z TYPE STRING.
    CONCATENATE   'MANDT LAND1 ZLSCH TEXT1 XEINZ XSCHK'
                  ' XWECH BLART XSWEC XORB' INTO column_T042Z.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_T042Z.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_T042Z IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_T042Z.
  ENDIF.
ENDIF.
ENDIF.
...
*T042ZT Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T042ZT'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_T042ZT = 'Yes'.
    DATA: COLUMN_T042ZT TYPE STRING.
    DATA TABLE_T042ZT TYPE STRING.
    CONCATENATE   'MANDT SPRAS LAND1'
                  ' ZLSCH TEXT2' INTO column_T042ZT.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_T042ZT.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_T042ZT IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_T042ZT.
  ENDIF.
ENDIF.
ENDIF.
...
*T16FT Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T16FT'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_T16FT = 'Yes'.
    DATA: COLUMN_T16FT TYPE STRING.
    DATA TABLE_T16FT TYPE STRING.
    CONCATENATE   'MANDT SPRAS FRGGR'
                  ' FRGSX FRGXT' INTO column_T16FT.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_T16FT.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_T16FT IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_T16FT.
  ENDIF.
ENDIF.
ENDIF.

*/TMWFLOW/IMPSTAN Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = '/TMWFLOW/IMPSTAN'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_/TMWFLOW/IMPSTAN = 'Yes'.
    DATA: COLUMN_/TMWFLOW/IMPSTAN TYPE STRING.
    DATA TABLE_/TMWFLOW/IMPSTAN TYPE STRING.
    CONCATENATE   'SUB_LANDSCAPE_ID BRANCH_NAME CYCLE_TYPE SYS_NAME SYS_TYPE'
                  ' SYS_CLIENT SYS_ROLE PROC_TYPE STSMA STATUS' INTO column_/TMWFLOW/IMPSTAN.

    TABLE_/TMWFLOW/IMPSTAN = '/TMWFLOW/IMPSTAN'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_/TMWFLOW/IMPSTAN)
   FROM (TABLE_/TMWFLOW/IMPSTAN)
  .
DO.
  "fetch data in chunks for every 'maxsize' number of records
    FETCH NEXT CURSOR S_CURSOR
    APPENDING TABLE IT_/TMWFLOW/IMPSTAN
    PACKAGE SIZE MAXSIZE.
    IF SY-SUBRC <> 0.
      CLOSE CURSOR S_CURSOR.
      EXIT.
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_/TMWFLOW/IMPSTAN.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_/TMWFLOW/IMPSTAN IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_/TMWFLOW/IMPSTAN.
  ENDIF.
ENDIF.
ENDIF.

*/TMWFLOW/PROJCTC Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = '/TMWFLOW/PROJCTC'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_/TMWFLOW/PROJCTC = 'Yes'.
    DATA: COLUMN_/TMWFLOW/PROJCTC TYPE STRING.
    DATA TABLE_/TMWFLOW/PROJCTC TYPE STRING.
    CONCATENATE   'TASKLIST PROJECT_TYPE SMI_PROJECT PARENT_TASKLIST RESP_USER'
                  ' CREATED_DATE STATUS ORIGINATOR_KEY' INTO column_/TMWFLOW/PROJCTC.

    TABLE_/TMWFLOW/PROJCTC = '/TMWFLOW/PROJCTC'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_/TMWFLOW/PROJCTC)
   FROM (TABLE_/TMWFLOW/PROJCTC)
  .
DO.
  "fetch data in chunks for every 'maxsize' number of records
    FETCH NEXT CURSOR S_CURSOR
    APPENDING TABLE IT_/TMWFLOW/PROJCTC
    PACKAGE SIZE MAXSIZE.
    IF SY-SUBRC <> 0.
      CLOSE CURSOR S_CURSOR.
      EXIT.
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_/TMWFLOW/PROJCTC.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_/TMWFLOW/PROJCTC IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_/TMWFLOW/PROJCTC.
  ENDIF.
ENDIF.
ENDIF.

*/TMWFLOW/PROJCTT Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = '/TMWFLOW/PROJCTT'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_/TMWFLOW/PROJCTT = 'Yes'.
    DATA: COLUMN_/TMWFLOW/PROJCTT TYPE STRING.
    DATA TABLE_/TMWFLOW/PROJCTT TYPE STRING.
    CONCATENATE   'LANGU TASKLIST'
                  ' DESCRIPTION' INTO column_/TMWFLOW/PROJCTT.

    TABLE_/TMWFLOW/PROJCTT = '/TMWFLOW/PROJCTT'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_/TMWFLOW/PROJCTT)
   FROM (TABLE_/TMWFLOW/PROJCTT)
   WHERE LANGU = 'EN'
  .
DO.
  "fetch data in chunks for every 'maxsize' number of records
    FETCH NEXT CURSOR S_CURSOR
    APPENDING TABLE IT_/TMWFLOW/PROJCTT
    PACKAGE SIZE MAXSIZE.
    IF SY-SUBRC <> 0.
      CLOSE CURSOR S_CURSOR.
      EXIT.
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_/TMWFLOW/PROJCTT.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_/TMWFLOW/PROJCTT IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_/TMWFLOW/PROJCTT.
  ENDIF.
ENDIF.
ENDIF.
...
*T043 Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T043'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_T043 = 'Yes'.
    DATA: COLUMN_T043 TYPE STRING.
    DATA TABLE_T043 TYPE STRING.
    CONCATENATE   'MANDT USNAM'
                  ' RFPRO INVPR' INTO column_T043.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_T043.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_T043 IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_T043.
  ENDIF.
ENDIF.
ENDIF.
...
*T683U Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T683U'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_T683U = 'Yes'.
    DATA: COLUMN_T683U TYPE STRING.
    DATA TABLE_T683U TYPE STRING.
    CONCATENATE   'MANDT SPRAS KVEWE KAPPL'
                  ' KALSM VTEXT' INTO column_T683U.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_T683U.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_T683U IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_T683U.
  ENDIF.
ENDIF.
ENDIF.
...
*T685T Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'T685T'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_T685T = 'Yes'.
    DATA: COLUMN_T685T TYPE STRING.
    DATA TABLE_T685T TYPE STRING.
    CONCATENATE   'MANDT SPRAS KVEWE KAPPL'
                  ' KSCHL VTEXT' INTO column_T685T.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_T685T.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_T685T IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_T685T.
  ENDIF.
ENDIF.
ENDIF.
...
*CABNT Extraction
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'CABNT'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

   IF ISTABLE_CABNT = 'Yes'.
    DATA: COLUMN_CABNT TYPE STRING.
    DATA TABLE_CABNT TYPE STRING.
    CONCATENATE   'MANDT ATINN SPRAS'
                  ' ADZHL ATBEZ' INTO column_CABNT.
...
DO.
  "fetch data in chunks for every 'maxsize' number of records
    ...
    ELSE.
      IF rb_fore IS INITIAL.
        PERFORM SUB_DOWNLOAD_CABNT.
      ENDIF.
    ENDIF.

  ENDDO.
  .
  IF rb_fore IS NOT INITIAL AND IT_CABNT IS NOT INITIAL.
  PERFORM SUB_DOWNLOAD_CABNT.
  ENDIF.
ENDIF.
ENDIF.
...
*Added new table GRACEXCLUDEDOBJS (GRC Specific) (Release 20.0)
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'GRACEXCLUDEDOBJS'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

  IF ISTABLE_GRACEXCLUDEDOBJS = 'Yes'.

    DATA: COLUMN_GRACEXCLUDEDOBJS TYPE STRING.
    DATA TABLE_GRACEXCLUDEDOBJS TYPE STRING.
    COLUMN_GRACEXCLUDEDOBJS = 'MANDT OBJECTTYPE FROMVAL TOVAL CONNECTOR ACTIVE COMMENTS'.
    TABLE_GRACEXCLUDEDOBJS = 'GRACEXCLUDEDOBJS'.
   SELECT
       (COLUMN_GRACEXCLUDEDOBJS)
   FROM (TABLE_GRACEXCLUDEDOBJS)
   INTO TABLE IT_GRACEXCLUDEDOBJS
   WHERE MANDT =  S_PCLIEN.
      lv_file = 'GRACEXCLUDEDOBJS.ACTT'.
      ls_table = 'MANDT'.
      DESCRIBE FIELD wa_GRACEXCLUDEDOBJS-MANDT
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'OBJECTTYPE' INTO ls_table.
      DESCRIBE FIELD wa_GRACEXCLUDEDOBJS-OBJECTTYPE
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'FROMVAL' INTO ls_table.
      DESCRIBE FIELD wa_GRACEXCLUDEDOBJS-FROMVAL
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'TOVAL' INTO ls_table.
      DESCRIBE FIELD wa_GRACEXCLUDEDOBJS-TOVAL
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'CONNECTOR' INTO ls_table.
      DESCRIBE FIELD wa_GRACEXCLUDEDOBJS-CONNECTOR
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'ACTIVE' INTO ls_table.
      DESCRIBE FIELD wa_GRACEXCLUDEDOBJS-ACTIVE
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'COMMENTS' INTO ls_table.
      DESCRIBE FIELD wa_GRACEXCLUDEDOBJS-COMMENTS
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      lv_GRACEXCLUDEDOBJS = LINES( it_GRACEXCLUDEDOBJS ) .
      IF sy-subrc = 0.
        PERFORM conv_table_to_text TABLES it_GRACEXCLUDEDOBJS
        USING lv_file
        ls_table .
        FREE it_GRACEXCLUDEDOBJS .
        CLEAR lt_table .
        CLEAR ls_table .
      ELSE.
        APPEND ls_table TO lt_table .
...
*Added new table GRACFFLOG (GRC-Specific) (Release 20.0)
READ TABLE p_tables WITH KEY sign = 'I' option = 'EQ' low = 'GRACFFLOG'.
IF ck_pop <> 'X' OR sy-subrc <> 0.

  IF ISTABLE_GRACFFLOG = 'Yes'.
    DATA: COLUMN_GRACFFLOG TYPE STRING.
    DATA TABLE_GRACFFLOG TYPE STRING.
    CONCATENATE 'MANDT FFLOG_ID FFOBJECT CONNECTOR FF_USER LOGON_TIME'
                ' FF_OWNER WORKFLOW_SENT REASON_TEXT'
                ' UPDATED_AT LOG_FROM_PLUGIN LOGOFF_TIME LOGIN_STATUS'  INTO COLUMN_GRACFFLOG.
    TABLE_GRACFFLOG = 'GRACFFLOG'.
OPEN CURSOR WITH HOLD s_cursor FOR
   SELECT
       (COLUMN_GRACFFLOG)
   FROM (TABLE_GRACFFLOG)
   WHERE MANDT =  S_PCLIEN
  .
DO.
  "fetch data in chunks for every 'maxsize' number of records
        FETCH NEXT CURSOR s_cursor
        APPENDING TABLE it_GRACFFLOG
        PACKAGE SIZE maxsize.
        IF sy-subrc <> 0.
          CLOSE CURSOR s_cursor.
          EXIT.
        ELSE.
         IF rb_fore IS INITIAL.
           PERFORM SUB_DOWNLOAD_GRACFFLOG.
         ENDIF.
        ENDIF.
      ENDDO.
      .
      .
        IF rb_fore IS NOT INITIAL AND IT_GRACFFLOG IS NOT INITIAL.
          PERFORM SUB_DOWNLOAD_GRACFFLOG.
        ENDIF.
      ENDIF.
ENDIF.
...
PERFORM download_gui TABLES lt_table USING lv_file.
  ...
  CLEAR LS_TABLE.
  IF ISTABLE_RSZCOMPDIR = 'Yes'.
   CONCATENATE 'RSZCOMPDIR' '|^|' lv_RSZCOMPDIR  INTO ls_table .
   APPEND ls_table TO lt_table .
   CLEAR ls_table.
  ENDIF.
  IF ISTABLE_RSZCOMPIC = 'Yes'.
   CONCATENATE 'RSZCOMPIC' '|^|' lv_RSZCOMPIC  INTO ls_table .
   APPEND ls_table TO lt_table .
   CLEAR ls_table.
  ENDIF.
  IF ISTABLE_RSZELTTXT = 'Yes'.
   CONCATENATE 'RSZELTTXT' '|^|' lv_RSZELTTXT  INTO ls_table .
   APPEND ls_table TO lt_table .
   CLEAR ls_table.
  ENDIF.
  IF ISTABLE_RSZELTDIR = 'Yes'.
   CONCATENATE 'RSZELTDIR' '|^|' lv_RSZELTDIR  INTO ls_table .
   APPEND ls_table TO lt_table .
   CLEAR ls_table.
  ENDIF.
  ...
  CLEAR LS_TABLE.
IF ISTABLE_USRACL = 'Yes'.
  CONCATENATE 'USRACL' '|^|' lv_USRACL  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
...
ENDIF.
IF ISTABLE_AUSP = 'Yes'.
    CONCATENATE 'AUSP' '|^|' lv_AUSP  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
IF ISTABLE_TVUVF = 'Yes'.
    CONCATENATE 'TVUVF' '|^|' LV_TVUVF    INTO LS_TABLE .
    APPEND LS_TABLE TO LT_TABLE .
    CLEAR LS_TABLE.
  ENDIF.

  IF ISTABLE_TVUVS = 'Yes'.
    CONCATENATE 'TVUVS' '|^|' lv_TVUVS  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.

IF ISTABLE_T049L = 'Yes'.
    CONCATENATE 'T049L' '|^|' lv_T049L  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.

IF ISTABLE_T042I = 'Yes'.
    CONCATENATE 'T042I' '|^|' lv_T042I  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.

IF ISTABLE_T16FW = 'Yes'.
  CONCATENATE 'T16FW' '|^|' lv_T16FW  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_T16FD = 'Yes'.
  CONCATENATE 'T16FD' '|^|' lv_T16FD  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_T16FH = 'Yes'.
  CONCATENATE 'T16FH' '|^|' lv_T16FH  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_T169L = 'Yes'.
  CONCATENATE 'T169L' '|^|' lv_T169L  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.

IF ISTABLE_T025T = 'Yes'.
  CONCATENATE 'T025T' '|^|' lv_T025T  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_REGUH = 'Yes'.
  CONCATENATE 'REGUH' '|^|' lv_REGUH  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_T014 = 'Yes'.
  CONCATENATE 'T014' '|^|' lv_T014  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_T078K = 'Yes'.
  CONCATENATE 'T078K' '|^|' lv_T078K  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_T079M = 'Yes'.
  CONCATENATE 'T079M' '|^|' lv_T079M  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_TVKWZ = 'Yes'.
  CONCATENATE 'TVKWZ' '|^|' lv_TVKWZ  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_TKES1 = 'Yes'.
  CONCATENATE 'TKES1' '|^|' lv_TKES1  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_TKES2 = 'Yes'.
  CONCATENATE 'TKES2' '|^|' lv_TKES2  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_ANKA = 'Yes'.
...
ENDIF.
IF ISTABLE_T100_SM = 'Yes'.
  CONCATENATE 'T100_SM' '|^|' lv_T100_sm  INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF.
IF ISTABLE_T100C = 'Yes'.
...
ENDIF .
IF ISTABLE_TB003T = 'Yes'.
  CONCATENATE 'TB003T' '|^|' lv_TB003T INTO ls_table.
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
ENDIF .
IF ISTABLE_TBAER = 'Yes'.
...
ENDIF .
IF ISTABLE_TVKVT = 'Yes'.
  CONCATENATE 'TVKVT' '|^|' LV_TVKVT  INTO LS_TABLE .
  APPEND LS_TABLE TO LT_TABLE .
  CLEAR LS_TABLE.
ENDIF .
IF ISTABLE_TVKDT = 'Yes'.
  CONCATENATE 'TVKDT' '|^|' LV_TVKDT  INTO LS_TABLE .
  APPEND LS_TABLE TO LT_TABLE .
  CLEAR LS_TABLE.
ENDIF .
IF ISTABLE_T184 = 'Yes'.
  CONCATENATE 'T184' '|^|' LV_T184  INTO LS_TABLE .
  APPEND LS_TABLE TO LT_TABLE .
  CLEAR LS_TABLE.
ENDIF .
...
*#ACTTSAP12.0 Release 12.0 Changes Begin*
  ...
  ENDIF.
  IF istable_TJ30 = 'Yes'.
    CONCATENATE 'TJ30' '|^|' lv_TJ30  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_TJ30T = 'Yes'.
    CONCATENATE 'TJ30T' '|^|' lv_TJ30T  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_TSOCM_BPFCT_DEF2 = 'Yes'.
    CONCATENATE 'TSOCM_BPFCT_DEF2' '|^|' lv_TSOCM_BPFCT_DEF2  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_TSOCM_COND_MAPP = 'Yes'.
    CONCATENATE 'TSOCM_COND_MAPP' '|^|' lv_TSOCM_COND_MAPP  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_CRMC_PARTNER_FT = 'Yes'.
    CONCATENATE 'CRMC_PARTNER_FT' '|^|' lv_CRMC_PARTNER_FT  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_T012K = 'Yes'.
    CONCATENATE 'T012K' '|^|' lv_T012K  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_KNB1 = 'Yes'.
    CONCATENATE 'KNB1' '|^|' lv_KNB1  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_T042Z = 'Yes'.
    CONCATENATE 'T042Z' '|^|' lv_T042Z  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_T042ZT = 'Yes'.
    CONCATENATE 'T042ZT' '|^|' lv_T042ZT  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_T16FT = 'Yes'.
    CONCATENATE 'T16FT' '|^|' lv_T16FT  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_/TMWFLOW/IMPSTAN = 'Yes'.
    CONCATENATE '/TMWFLOW/IMPSTAN' '|^|' lv_/TMWFLOW/IMPSTAN  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_/TMWFLOW/PROJCTC = 'Yes'.
    CONCATENATE '/TMWFLOW/PROJCTC' '|^|' lv_/TMWFLOW/PROJCTC  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_/TMWFLOW/PROJCTT = 'Yes'.
    CONCATENATE '/TMWFLOW/PROJCTT' '|^|' lv_/TMWFLOW/PROJCTT  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_T043 = 'Yes'.
    CONCATENATE 'T043' '|^|' lv_T043  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_T683U = 'Yes'.
    CONCATENATE 'T683U' '|^|' lv_T683U  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_T685T = 'Yes'.
    CONCATENATE 'T685T' '|^|' lv_T685T  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_CABNT = 'Yes'.
    CONCATENATE 'CABNT' '|^|' lv_CABNT  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_GRACEXCLUDEDOBJS = 'Yes'.
   CONCATENATE 'GRACEXCLUDEDOBJS' '|^|' lv_GRACEXCLUDEDOBJS  INTO ls_table .
   APPEND ls_table TO lt_table .
   CLEAR ls_table.
  ENDIF.
  IF istable_GRACFFOWNER = 'Yes'.
  ...
  ENDIF.
  IF istable_GRACFFLOG = 'Yes'.
    CONCATENATE 'GRACFFLOG' '|^|' lv_GRACFFLOG  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  IF istable_TACTT = 'Yes'.
    CONCATENATE 'TACTT' '|^|' lv_TACTT  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.

  IF istable_T001K = 'Yes'.
    CONCATENATE 'T001K' '|^|' lv_T001K  INTO ls_table .
    APPEND ls_table TO lt_table .
    CLEAR ls_table.
  ENDIF.
  ...
  IF ISTABLE_SEC_POLICY_CUSTT = 'Yes'.
    ...
    CLEAR LS_TABLE.
  ENDIF.
  CONCATENATE 'TPALOG_ALL' '|^|' LV_TPALOG  INTO LS_TABLE .
  APPEND LS_TABLE TO LT_TABLE .
  CLEAR LS_TABLE.
...
*moved by piyush to the end
*** SV 02102020 -- SM21 EXTRACTION LOGIC
** IN CASE OF CHECKBOX OF SM20 IS SELECTED, THEN EXTRACT
** SM20
  IF ck_sm2 IS NOT INITIAL.

      DATA : lv_unit2 TYPE dlvunit.
      DATA : lv_cv2 TYPE c.
      SELECT SINGLE NAME INTO lv_unit2
         FROM TRDIR WHERE NAME = 'RSAU_READ_LOG'.
     IF sy-subrc = 0.
      lv_cv2 = 'X'.
     ENDIF.
    IF lv_cv2 <> 'X'.
...
*Begin of changes by gentelbh on 07/04.
  SELECT MANDT
         ...
         CLIENT SPECIFIED
         INTO TABLE T_USR02
***    Filter being introduced to avoid security issues arising due to selection of column PASSCODE -  GAJHA  ******
             WHERE BNAME IN ('SAP*','DDIC', 'SAPCPIC', 'EARLYWATCH', 'TMSADM').
***    Filter being introduced to avoid security issues arising due to selection of column PASSCODE -  GAJHA  ******
...
FORM sub_download_DD09L .
  ...
  lv_DD09L = lv_DD09L + LINES( it_DD09L ) .
  IF lv_DD09L GT 0.
...
*&---------------------------------------------------------------------*
*&      Form  SUB_DOWNLOAD_RSZCOMPDIR
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_RSZCOMPDIR .
  STATICS : lv_no_head TYPE c.
lv_file = 'RSZCOMPDIR.ACTT'.
if lv_no_head IS INITIAL.

    LV_NO_HEAD = 'X'.
  ls_table = 'COMPUID'.
DESCRIBE FIELD WA_RSZCOMPDIR-COMPUID
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'OBJVERS' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-OBJVERS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'COMPID' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-COMPID
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'VERSION' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-VERSION
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'COMPDIM' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-COMPDIM
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'OBJSTAT' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-OBJSTAT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'CONTREL' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-CONTREL
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'CONTTIMESTMP' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-CONTTIMESTMP
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'OWNER' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-OWNER
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'BWAPPL' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-BWAPPL
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ACTIVFL' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-ACTIVFL
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TIMESTMP' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-TIMESTMP
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TSTPNM' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-TSTPNM
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TSTPDAT' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-TSTPDAT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TSTPTIM' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-TSTPTIM
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'LASTUSED' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-LASTUSED
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'CREATED' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-CREATED
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'CHANGED_WITH' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-CHANGED_WITH
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'INFOAREA' into ls_table.
DESCRIBE FIELD WA_RSZCOMPDIR-INFOAREA
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_RSZCOMPDIR = lv_RSZCOMPDIR + LINES( it_RSZCOMPDIR ) .
  IF lv_RSZCOMPDIR GT 0.
perform conv_table_to_text tables it_RSZCOMPDIR
using lv_file
 ls_table .
FREE IT_RSZCOMPDIR.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_RSZCOMPDIR
...
*  <--  p2        text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_GRACFFLOG .

  STATICS : lv_no_head TYPE c.

  lv_file = 'GRACFFLOG.ACTT'.
  IF lv_no_head IS INITIAL.

    lv_no_head = 'X'.
    ls_table = 'MANDT'.
      DESCRIBE FIELD wa_GRACFFLOG-MANDT
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'FFLOG_ID' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-FFLOG_ID
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'FFOBJECT' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-FFOBJECT
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'CONNECTOR' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-CONNECTOR
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'FF_USER' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-FF_USER
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'LOGON_TIME' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-LOGON_TIME
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'FF_OWNER' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-FF_OWNER
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'WORKFLOW_SENT' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-WORKFLOW_SENT
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'REASON_TEXT' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-REASON_TEXT
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'UPDATED_AT' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-UPDATED_AT
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'LOG_FROM_PLUGIN' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-LOG_FROM_PLUGIN
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'LOGOFF_TIME' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-LOGOFF_TIME
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
      CONCATENATE ls_table '|^|' 'LOGIN_STATUS' INTO ls_table.
      DESCRIBE FIELD wa_GRACFFLOG-LOGIN_STATUS
      TYPE lv_type LENGTH lv_len IN BYTE MODE.
      CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
   ENDIF.

    lv_gracfflog = lv_gracfflog + LINES( it_gracfflog ) .
  IF lv_gracfflog GT 0.
    PERFORM conv_table_to_text TABLES it_gracfflog
    USING lv_file
    ls_table .
    FREE it_gracfflog .
    CLEAR lt_table .
...
ENDFORM.                    " SUB_DOWNLOAD_GRACFFLOG
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_AUSP-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'OBJEK' into ls_table.
DESCRIBE FIELD WA_AUSP-OBJEK
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATINN' into ls_table.
DESCRIBE FIELD WA_AUSP-ATINN
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATZHL' into ls_table.
DESCRIBE FIELD WA_AUSP-ATZHL
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'MAFID' into ls_table.
DESCRIBE FIELD WA_AUSP-MAFID
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'KLART' into ls_table.
DESCRIBE FIELD WA_AUSP-KLART
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ADZHL' into ls_table.
DESCRIBE FIELD WA_AUSP-ADZHL
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATWRT' into ls_table.
DESCRIBE FIELD WA_AUSP-ATWRT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATFLV' into ls_table.
DESCRIBE FIELD WA_AUSP-ATFLV
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATFLB' into ls_table.
DESCRIBE FIELD WA_AUSP-ATFLB
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATAW1' into ls_table.
DESCRIBE FIELD WA_AUSP-ATAW1
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATCOD' into ls_table.
DESCRIBE FIELD WA_AUSP-ATCOD
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATTLV' into ls_table.
DESCRIBE FIELD WA_AUSP-ATTLV
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATTLB' into ls_table.
DESCRIBE FIELD WA_AUSP-ATTLB
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATPRZ' into ls_table.
DESCRIBE FIELD WA_AUSP-ATPRZ
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATINC' into ls_table.
DESCRIBE FIELD WA_AUSP-ATINC
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATAUT' into ls_table.
DESCRIBE FIELD WA_AUSP-ATAUT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'AENNR' into ls_table.
DESCRIBE FIELD WA_AUSP-AENNR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'DATUV' into ls_table.
DESCRIBE FIELD WA_AUSP-DATUV
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'LKENZ' into ls_table.
DESCRIBE FIELD WA_AUSP-LKENZ
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ATZIS' into ls_table.
DESCRIBE FIELD WA_AUSP-ATZIS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_AUSP = lv_AUSP + LINES( it_AUSP ) .
  IF lv_AUSP GT 0.
perform conv_table_to_text tables it_AUSP
using lv_file
 ls_table .
FREE IT_AUSP.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_AUSP


*&      Form  SUB_DOWNLOAD_TVUVF
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_TVUVF .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_TVUVF-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FEHGR' into ls_table.
DESCRIBE FIELD WA_TVUVF-FEHGR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TBNAM' into ls_table.
DESCRIBE FIELD WA_TVUVF-TBNAM
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FDNAM' into ls_table.
DESCRIBE FIELD WA_TVUVF-FDNAM
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'STATG' into ls_table.
DESCRIBE FIELD WA_TVUVF-STATG
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FCODE' into ls_table.
DESCRIBE FIELD WA_TVUVF-FCODE
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'MSGKZ' into ls_table.
DESCRIBE FIELD WA_TVUVF-MSGKZ
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SORTF' into ls_table.
DESCRIBE FIELD WA_TVUVF-SORTF
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_TVUVF = lv_TVUVF + LINES( it_TVUVF ) .
  IF lv_TVUVF GT 0.
perform conv_table_to_text tables it_TVUVF
using lv_file
 ls_table .
FREE IT_TVUVF.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_TVUVF

*&      Form  SUB_DOWNLOAD_TVUVS
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_TVUVS .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_TVUVS-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'STATG' into ls_table.
DESCRIBE FIELD WA_TVUVS-STATG
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UVALL' into ls_table.
DESCRIBE FIELD WA_TVUVS-UVALL
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UVVLK' into ls_table.
DESCRIBE FIELD WA_TVUVS-UVVLK
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UVFAK' into ls_table.
DESCRIBE FIELD WA_TVUVS-UVFAK
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UVPRS' into ls_table.
DESCRIBE FIELD WA_TVUVS-UVPRS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UVWAK' into ls_table.
DESCRIBE FIELD WA_TVUVS-UVWAK
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UVPIK' into ls_table.
DESCRIBE FIELD WA_TVUVS-UVPIK
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UVPAK' into ls_table.
DESCRIBE FIELD WA_TVUVS-UVPAK
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UVGEK' into ls_table.
DESCRIBE FIELD WA_TVUVS-UVGEK
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_TVUVS = lv_TVUVS + LINES( it_TVUVS ) .
  IF lv_TVUVS GT 0.
perform conv_table_to_text tables it_TVUVS
using lv_file
 ls_table .
FREE IT_TVUVS.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_TVUVS


*&      Form  SUB_DOWNLOAD_T049L
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_T049L .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_T049L-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'BUKRS' into ls_table.
DESCRIBE FIELD WA_T049L-BUKRS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'LOCKB' into ls_table.
DESCRIBE FIELD WA_T049L-LOCKB
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'HBKID' into ls_table.
DESCRIBE FIELD WA_T049L-HBKID
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'LCKNR' into ls_table.
DESCRIBE FIELD WA_T049L-LCKNR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ADRNR' into ls_table.
DESCRIBE FIELD WA_T049L-ADRNR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_T049L = lv_T049L + LINES( it_T049L ) .
  IF lv_T049L GT 0.
perform conv_table_to_text tables it_T049L
using lv_file
 ls_table .
FREE IT_T049L.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_T049L



*&      Form  SUB_DOWNLOAD_T042I
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_T042I .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_T042I-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ZBUKR' into ls_table.
DESCRIBE FIELD WA_T042I-ZBUKR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'HBKID' into ls_table.
DESCRIBE FIELD WA_T042I-HBKID
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ZLSCH' into ls_table.
DESCRIBE FIELD WA_T042I-ZLSCH
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'WAERS' into ls_table.
DESCRIBE FIELD WA_T042I-WAERS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'HKTID' into ls_table.
DESCRIBE FIELD WA_T042I-HKTID
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UKONT' into ls_table.
DESCRIBE FIELD WA_T042I-UKONT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'VKONT' into ls_table.
DESCRIBE FIELD WA_T042I-VKONT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'GEBKZ' into ls_table.
DESCRIBE FIELD WA_T042I-GEBKZ
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'GSBER' into ls_table.
DESCRIBE FIELD WA_T042I-GSBER
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.

ENDIF.
lv_T042I = lv_T042I + LINES( it_T042I ) .
  IF lv_T042I GT 0.
perform conv_table_to_text tables it_T042I
using lv_file
 ls_table .
FREE IT_T042I.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_T042I


*&      Form  SUB_DOWNLOAD_T16FW
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_T16FW .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_T16FW-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FRGGR' into ls_table.
DESCRIBE FIELD WA_T16FW-FRGGR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FRGCO' into ls_table.
DESCRIBE FIELD WA_T16FW-FRGCO
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'WERKS' into ls_table.
DESCRIBE FIELD WA_T16FW-WERKS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'OTYPE' into ls_table.
DESCRIBE FIELD WA_T16FW-OTYPE
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'OBJID' into ls_table.
DESCRIBE FIELD WA_T16FW-OBJID
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_T16FW = lv_T16FW + LINES( it_T16FW ) .
  IF lv_T16FW GT 0.
perform conv_table_to_text tables it_T16FW
using lv_file
 ls_table .
FREE IT_T16FW.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_T16FW


*&      Form  SUB_DOWNLOAD_T16FD
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_T16FD .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_T16FD-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SPRAS' into ls_table.
DESCRIBE FIELD WA_T16FD-SPRAS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FRGGR' into ls_table.
DESCRIBE FIELD WA_T16FD-FRGGR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FRGCO' into ls_table.
DESCRIBE FIELD WA_T16FD-FRGCO
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FRGCT' into ls_table.
DESCRIBE FIELD WA_T16FD-FRGCT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_T16FD = lv_T16FD + LINES( it_T16FD ) .
  IF lv_T16FD GT 0.
perform conv_table_to_text tables it_T16FD
using lv_file
 ls_table .
FREE IT_T16FD.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_T16FD


*&      Form  SUB_DOWNLOAD_T16FH
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_T16FH .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_T16FH-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SPRAS' into ls_table.
DESCRIBE FIELD WA_T16FH-SPRAS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FRGGR' into ls_table.
DESCRIBE FIELD WA_T16FH-FRGGR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FRGGT' into ls_table.
DESCRIBE FIELD WA_T16FH-FRGGT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_T16FH = lv_T16FH + LINES( it_T16FH ) .
  IF lv_T16FH GT 0.
perform conv_table_to_text tables it_T16FH
using lv_file
 ls_table .
FREE IT_T16FH.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_T16FH


*&      Form  SUB_DOWNLOAD_T169L
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_T169L .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_T169L-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'BUKRS' into ls_table.
DESCRIBE FIELD WA_T169L-BUKRS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TOGRR' into ls_table.
DESCRIBE FIELD WA_T169L-TOGRR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TEXT' into ls_table.
DESCRIBE FIELD WA_T169L-TEXT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'XAKZPU' into ls_table.
DESCRIBE FIELD WA_T169L-XAKZPU
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TAKZPU' into ls_table.
DESCRIBE FIELD WA_T169L-TAKZPU
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'XAKZPB' into ls_table.
DESCRIBE FIELD WA_T169L-XAKZPB
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TAKZPB' into ls_table.
DESCRIBE FIELD WA_T169L-TAKZPB
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'XAKZPP' into ls_table.
DESCRIBE FIELD WA_T169L-XAKZPP
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TAKZPP' into ls_table.
DESCRIBE FIELD WA_T169L-TAKZPP
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'XAKZNU' into ls_table.
DESCRIBE FIELD WA_T169L-XAKZNU
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TAKZNU' into ls_table.
DESCRIBE FIELD WA_T169L-TAKZNU
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'XAKZNB' into ls_table.
DESCRIBE FIELD WA_T169L-XAKZNB
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TAKZNB' into ls_table.
DESCRIBE FIELD WA_T169L-TAKZNB
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'XAKZNP' into ls_table.
DESCRIBE FIELD WA_T169L-XAKZNP
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TAKZNP' into ls_table.
DESCRIBE FIELD WA_T169L-TAKZNP
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'XRKUER' into ls_table.
DESCRIBE FIELD WA_T169L-XRKUER
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TRKPB' into ls_table.
DESCRIBE FIELD WA_T169L-TRKPB
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'XRKPP' into ls_table.
DESCRIBE FIELD WA_T169L-XRKPP
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TRKPP' into ls_table.
DESCRIBE FIELD WA_T169L-TRKPP
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_T169L = lv_T169L + LINES( it_T169L ) .
  IF lv_T169L GT 0.
perform conv_table_to_text tables it_T169L
using lv_file
 ls_table .
FREE IT_T169L.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_T169L

*&      Form  SUB_DOWNLOAD_T025T
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_T025T .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_T025T-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SPRAS' into ls_table.
DESCRIBE FIELD WA_T025T-SPRAS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'BKLAS' into ls_table.
DESCRIBE FIELD WA_T025T-BKLAS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'BKBEZ' into ls_table.
DESCRIBE FIELD WA_T025T-BKBEZ
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_T025T = lv_T025T + LINES( it_T025T ) .
  IF lv_T025T GT 0.
perform conv_table_to_text tables it_T025T
using lv_file
 ls_table .
FREE IT_T025T.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_T025T


*&      Form  SUB_DOWNLOAD_REGUH
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_REGUH .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_REGUH-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'LAUFD' into ls_table.
DESCRIBE FIELD WA_REGUH-LAUFD
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'LAUFI' into ls_table.
DESCRIBE FIELD WA_REGUH-LAUFI
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'XVORL' into ls_table.
DESCRIBE FIELD WA_REGUH-XVORL
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ZBUKR' into ls_table.
DESCRIBE FIELD WA_REGUH-ZBUKR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'LIFNR' into ls_table.
DESCRIBE FIELD WA_REGUH-LIFNR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'KUNNR' into ls_table.
DESCRIBE FIELD WA_REGUH-KUNNR
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'RZAWE' into ls_table.
DESCRIBE FIELD WA_REGUH-RZAWE
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'UZAWE' into ls_table.
DESCRIBE FIELD WA_REGUH-UZAWE
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.

ENDIF.
lv_REGUH = lv_REGUH + LINES( it_REGUH ) .
  IF lv_REGUH GT 0.
perform conv_table_to_text tables it_REGUH
using lv_file
 ls_table .
FREE IT_REGUH.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_REGUH

*&      Form  SUB_DOWNLOAD_T014
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_T014 .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_T014-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'KKBER' into ls_table.
DESCRIBE FIELD WA_T014-KKBER
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'WAERS' into ls_table.
DESCRIBE FIELD WA_T014-WAERS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'STAFO' into ls_table.
DESCRIBE FIELD WA_T014-STAFO
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PERIV' into ls_table.
DESCRIBE FIELD WA_T014-PERIV
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'CTLPC' into ls_table.
DESCRIBE FIELD WA_T014-CTLPC
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'KLIMK' into ls_table.
DESCRIBE FIELD WA_T014-KLIMK
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SBGRP' into ls_table.
DESCRIBE FIELD WA_T014-SBGRP
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'ALLCC' into ls_table.
DESCRIBE FIELD WA_T014-ALLCC
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_T014 = lv_T014 + LINES( it_T014 ) .
  IF lv_T014 GT 0.
perform conv_table_to_text tables it_T014
using lv_file
 ls_table .
FREE IT_T014.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_T014

*&      Form  SUB_DOWNLOAD_T078K
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_T078K .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_T078K-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TCODE' into ls_table.
DESCRIBE FIELD WA_T078K-TCODE
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSA' into ls_table.
DESCRIBE FIELD WA_T078K-FAUSA
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSF' into ls_table.
DESCRIBE FIELD WA_T078K-FAUSF
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSM' into ls_table.
DESCRIBE FIELD WA_T078K-FAUSM
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUS1' into ls_table.
DESCRIBE FIELD WA_T078K-FAUS1
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSW' into ls_table.
DESCRIBE FIELD WA_T078K-FAUSW
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUST' into ls_table.
DESCRIBE FIELD WA_T078K-FAUST
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSG' into ls_table.
DESCRIBE FIELD WA_T078K-FAUSG
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSN' into ls_table.
DESCRIBE FIELD WA_T078K-FAUSN
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSX' into ls_table.
DESCRIBE FIELD WA_T078K-FAUSX
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSU' into ls_table.
DESCRIBE FIELD WA_T078K-FAUSU
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUS2' into ls_table.
DESCRIBE FIELD WA_T078K-FAUS2
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_T078K = lv_T078K + LINES( it_T078K ) .
  IF lv_T078K GT 0.
perform conv_table_to_text tables it_T078K
using lv_file
 ls_table .
FREE IT_T078K.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_T078K

*&---------------------------------------------------------------------*
*&      Form  SUB_DOWNLOAD_USRACL
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_USRACL .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_USRACL-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'BNAME' into ls_table.
DESCRIBE FIELD WA_USRACL-BNAME
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SNC_SID' into ls_table.
DESCRIBE FIELD WA_USRACL-SNC_SID
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'GUIFLAG' into ls_table.
DESCRIBE FIELD WA_USRACL-GUIFLAG
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'PNAME' into ls_table.
DESCRIBE FIELD WA_USRACL-PNAME
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'HNAME' into ls_table.
DESCRIBE FIELD WA_USRACL-HNAME
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'CNAME' into ls_table.
DESCRIBE FIELD WA_USRACL-CNAME
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'CDATE' into ls_table.
DESCRIBE FIELD WA_USRACL-CDATE
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'CTIME' into ls_table.
DESCRIBE FIELD WA_USRACL-CTIME
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'MNAME' into ls_table.
DESCRIBE FIELD WA_USRACL-MNAME
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'MDATE' into ls_table.
DESCRIBE FIELD WA_USRACL-MDATE
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'MTIME' into ls_table.
DESCRIBE FIELD WA_USRACL-MTIME
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_USRACL = lv_USRACL + LINES( it_USRACL ) .
  IF lv_USRACL GT 0.
perform conv_table_to_text tables it_USRACL
using lv_file
 ls_table .
FREE IT_USRACL.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_USRACL

*&      Form  SUB_DOWNLOAD_T079M
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_T079M .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_T079M-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'EKORG' into ls_table.
DESCRIBE FIELD WA_T079M-EKORG
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSM' into ls_table.
DESCRIBE FIELD WA_T078K-FAUSM
...
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUST' into ls_table.
DESCRIBE FIELD WA_T079M-FAUST
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSW' into ls_table.
DESCRIBE FIELD WA_T079M-FAUSW
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSN' into ls_table.
DESCRIBE FIELD WA_T079M-FAUSN
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSX' into ls_table.
DESCRIBE FIELD WA_T079M-FAUSX
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FAUSU' into ls_table.
DESCRIBE FIELD WA_T079M-FAUSU
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_T079M = lv_T079M + LINES( it_T079M ) .
  IF lv_T079M GT 0.
perform conv_table_to_text tables it_T079M
using lv_file
 ls_table .
FREE IT_T079M.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_T079M


*&      Form  SUB_DOWNLOAD_TVKWZ
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_TVKWZ .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_TVKWZ-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'VKORG' into ls_table.
DESCRIBE FIELD WA_TVKWZ-VKORG
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'VTWEG' into ls_table.
DESCRIBE FIELD WA_TVKWZ-VTWEG
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'WERKS' into ls_table.
DESCRIBE FIELD WA_TVKWZ-WERKS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_TVKWZ = lv_TVKWZ + LINES( it_TVKWZ ) .
  IF lv_TVKWZ GT 0.
perform conv_table_to_text tables it_TVKWZ
using lv_file
 ls_table .
FREE IT_TVKWZ.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_TVKWZ

*&      Form  SUB_DOWNLOAD_TKES1
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_TKES1 .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_TKES1-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'APPLCLASS' into ls_table.
DESCRIBE FIELD WA_TKES1-APPLCLASS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SUBCLASS' into ls_table.
DESCRIBE FIELD WA_TKES1-SUBCLASS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TABNAME' into ls_table.
DESCRIBE FIELD WA_TKES1-TABNAME
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FORM' into ls_table.
DESCRIBE FIELD WA_TKES1-FORM
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'GDATE' into ls_table.
DESCRIBE FIELD WA_TKES1-GDATE
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'MODCN' into ls_table.
DESCRIBE FIELD WA_TKES1-MODCN
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'MODNA' into ls_table.
DESCRIBE FIELD WA_TKES1-MODNA
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'MODDA' into ls_table.
DESCRIBE FIELD WA_TKES1-MODDA
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'MODTI' into ls_table.
DESCRIBE FIELD WA_TKES1-MODTI
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_TKES1 = lv_TKES1 + LINES( it_TKES1 ) .
  IF lv_TKES1 GT 0.
perform conv_table_to_text tables it_TKES1
using lv_file
 ls_table .
FREE IT_TKES1.
CLEAR lt_table .
...
CLEAR ls_table .
ENDIF.
ENDFORM.                     " Sub_Download_TKES1

*&      Form  SUB_DOWNLOAD_TKES2
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_TKES2 .
...
if lv_no_head IS INITIAL.
    ...
    LV_NO_HEAD = 'X'.
  ls_table = 'MANDT'.
DESCRIBE FIELD WA_TKES2-MANDT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SPRAS' into ls_table.
DESCRIBE FIELD WA_TKES2-SPRAS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'APPLCLASS' into ls_table.
DESCRIBE FIELD WA_TKES2-APPLCLASS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'SUBCLASS' into ls_table.
DESCRIBE FIELD WA_TKES2-SUBCLASS
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TABNAME' into ls_table.
DESCRIBE FIELD WA_TKES2-TABNAME
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'FORM' into ls_table.
DESCRIBE FIELD WA_TKES2-FORM
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TXTKZ' into ls_table.
DESCRIBE FIELD WA_TKES2-TXTKZ
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TXTMT' into ls_table.
DESCRIBE FIELD WA_TKES2-TXTMT
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
CONCATENATE ls_table '|^|' 'TXTLG' into ls_table.
DESCRIBE FIELD WA_TKES2-TXTLG
TYPE LV_TYPE LENGTH LV_LEN IN BYTE MODE.
CONCATENATE LS_TABLE ' [' LV_TYPE '(' LV_LEN ')]' INTO LS_TABLE.
ENDIF.
lv_TKES2 = lv_TKES2 + LINES( it_TKES2 ) .
  IF lv_TKES2 GT 0.
perform conv_table_to_text tables it_TKES2
using lv_file
 ls_table .
FREE IT_TKES2.
CLEAR lt_table .
...
*
    ...
    IF lv_lines GT maxsize OR lv_index EQ lv_num.
      ...
      IF lv_head IS INITIAL.
        ...
        CONCATENATE ls_table 'UserId [C(00300)]|^|Date [D(00016)]|^|Time [T(00012)]|^|' INTO ls_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_TJ30-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'STSMA' INTO ls_table.
    DESCRIBE FIELD wa_TJ30-STSMA
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'ESTAT' INTO ls_table.
    DESCRIBE FIELD wa_TJ30-ESTAT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'INIST' INTO ls_table.
    DESCRIBE FIELD wa_TJ30-INIST
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'STONR' INTO ls_table.
    DESCRIBE FIELD wa_TJ30-STONR
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'HSONR' INTO ls_table.
    DESCRIBE FIELD wa_TJ30-HSONR
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'LINEP' INTO ls_table.
    DESCRIBE FIELD wa_TJ30-LINEP
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'STATP' INTO ls_table.
    DESCRIBE FIELD wa_TJ30-STATP
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'BERSL' INTO ls_table.
    DESCRIBE FIELD wa_TJ30-BERSL
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'CRM_VRGNG' INTO ls_table.
    DESCRIBE FIELD wa_TJ30-CRM_VRGNG
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
*    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
*    CONCATENATE ls_table '|^|' 'TXT04' INTO ls_table.
*    DESCRIBE FIELD wa_TJ30-TXT04
*    TYPE lv_type LENGTH lv_len IN BYTE MODE.
*    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
*    CONCATENATE ls_table '|^|' 'TXT30' INTO ls_table.
*    DESCRIBE FIELD wa_TJ30-TXT30
*    TYPE lv_type LENGTH lv_len IN BYTE MODE.
*    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
*    CONCATENATE ls_table '|^|' 'LTEXT' INTO ls_table.
*    DESCRIBE FIELD wa_TJ30-LTEXT
*    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_TJ30 = lv_TJ30 + LINES( it_TJ30 ) .
  IF lv_TJ30 GT 0.
    PERFORM conv_table_to_text TABLES it_TJ30
    USING lv_file
    ls_table .
    FREE it_TJ30 .
    CLEAR lt_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_TJ30T-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'STSMA' INTO ls_table.
    DESCRIBE FIELD wa_TJ30T-STSMA
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'ESTAT' INTO ls_table.
    DESCRIBE FIELD wa_TJ30T-ESTAT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'SPRAS' INTO ls_table.
    DESCRIBE FIELD wa_TJ30T-SPRAS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'TXT04' INTO ls_table.
    DESCRIBE FIELD wa_TJ30T-TXT04
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'TXT30' INTO ls_table.
    DESCRIBE FIELD wa_TJ30T-TXT30
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'LTEXT' INTO ls_table.
    DESCRIBE FIELD wa_TJ30T-LTEXT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_TJ30T  = lv_TJ30T + LINES( it_TJ30T ) .
  IF lv_TJ30T GT 0.
    PERFORM conv_table_to_text TABLES it_TJ30T
    USING lv_file
    ls_table .
    FREE it_TJ30T .
    CLEAR lt_table .
...
*&---------------------------------------------------------------------*
*&      Form  SUB_DOWNLOAD_TSOCM_BPFCT_DEF2
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
STATICS : lv_no_head TYPE c.
  ...
  lv_file = 'TSOCM_BPFCT_DEF2.ACTT'.
  IF lv_no_head IS INITIAL.

    lv_no_head = 'X'.

    ls_table = 'CLIENT'.
    DESCRIBE FIELD wa_TSOCM_BPFCT_DEF2-CLIENT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'PROCESS_TYPE_F' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_BPFCT_DEF2-PROCESS_TYPE_F
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'USER_STATUS' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_BPFCT_DEF2-USER_STATUS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'PARTNER_FCT_F' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_BPFCT_DEF2-PARTNER_FCT_F
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'PARTNER_FCT_COPY' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_BPFCT_DEF2-PARTNER_FCT_COPY
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'PARTNER_FCT_CHEC' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_BPFCT_DEF2-PARTNER_FCT_CHEC
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_TSOCM_BPFCT_DEF2 = lv_TSOCM_BPFCT_DEF2 + LINES( it_TSOCM_BPFCT_DEF2 ) .
  IF lv_TSOCM_BPFCT_DEF2 GT 0.
    PERFORM conv_table_to_text TABLES it_TSOCM_BPFCT_DEF2
    USING lv_file
    ls_table .
    FREE it_TSOCM_BPFCT_DEF2 .
    CLEAR lt_table .
    CLEAR ls_table .
  ELSE.
    APPEND ls_table TO lt_table .
...
*&---------------------------------------------------------------------*
*&      Form  SUB_DOWNLOAD_TSOCM_COND_MAPP
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
STATICS : lv_no_head TYPE c.
  ...
  lv_file = 'TSOCM_COND_MAPP.ACTT'.
  IF lv_no_head IS INITIAL.

    lv_no_head = 'X'.

    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'PROC_TYPE' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-PROC_TYPE
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'STSMA' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-STSMA
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'USER_STATUS' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-USER_STATUS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'CHECK_ORDER' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-CHECK_ORDER
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'CONDITION_ID' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-CONDITION_ID
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'ARBGB' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-ARBGB
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'MSGNR' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-MSGNR
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'TYPE' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-TYPE
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'STATUS_RESET' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-STATUS_RESET
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'RESET_APPR' INTO ls_table.
    DESCRIBE FIELD wa_TSOCM_COND_MAPP-RESET_APPR
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_TSOCM_COND_MAPP = lv_TSOCM_COND_MAPP + LINES( it_TSOCM_COND_MAPP ) .
  IF lv_TSOCM_COND_MAPP GT 0.
    PERFORM conv_table_to_text TABLES it_TSOCM_COND_MAPP
    USING lv_file
    ls_table .
    FREE it_TSOCM_COND_MAPP .
    CLEAR lt_table .
    CLEAR ls_table .
  ELSE.
    APPEND ls_table TO lt_table .
...
*&      Form  SUB_DOWNLOAD_CRMC_PARTNER_FT
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
STATICS : lv_no_head TYPE c.
  ...
  lv_file = 'CRMC_PARTNER_FT.ACTT'.
  IF lv_no_head IS INITIAL.

    lv_no_head = 'X'.

    ls_table = 'CLIENT'.
    DESCRIBE FIELD wa_CRMC_PARTNER_FT-CLIENT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'PARTNER_FCT' INTO ls_table.
    DESCRIBE FIELD wa_CRMC_PARTNER_FT-PARTNER_FCT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'SPRAS' INTO ls_table.
    DESCRIBE FIELD wa_CRMC_PARTNER_FT-SPRAS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'DESCRIPTION' INTO ls_table.
    DESCRIBE FIELD wa_CRMC_PARTNER_FT-DESCRIPTION
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_CRMC_PARTNER_FT = lv_CRMC_PARTNER_FT + LINES( it_CRMC_PARTNER_FT ) .
  IF lv_CRMC_PARTNER_FT GT 0.
    PERFORM conv_table_to_text TABLES it_CRMC_PARTNER_FT
    USING lv_file
    ls_table .
    FREE it_CRMC_PARTNER_FT .
    CLEAR lt_table .
    CLEAR ls_table .
  ELSE.
    APPEND ls_table TO lt_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_T012K-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'BUKRS' INTO ls_table.
    DESCRIBE FIELD wa_T012K-BUKRS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'HBKID' INTO ls_table.
    DESCRIBE FIELD wa_T012K-HBKID
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'HKTID' INTO ls_table.
    DESCRIBE FIELD wa_T012K-HKTID
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'BANKN' INTO ls_table.
    DESCRIBE FIELD wa_T012K-BANKN
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'WAERS' INTO ls_table.
    DESCRIBE FIELD wa_T012K-WAERS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'HKONT' INTO ls_table.
    DESCRIBE FIELD wa_T012K-HKONT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_T012K = lv_T012K + LINES( it_T012K ) .
  IF lv_T012K GT 0.
    PERFORM conv_table_to_text TABLES it_T012K
    USING lv_file
    ls_table .
    FREE it_T012K .
    CLEAR lt_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_KNB1-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'KUNNR' INTO ls_table.
    DESCRIBE FIELD wa_KNB1-KUNNR
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'BUKRS' INTO ls_table.
    DESCRIBE FIELD wa_KNB1-BUKRS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'TOGRU' INTO ls_table.
    DESCRIBE FIELD wa_KNB1-TOGRU
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_KNB1 = lv_KNB1 + LINES( it_KNB1 ) .
  IF lv_KNB1 GT 0.
    PERFORM conv_table_to_text TABLES it_KNB1
    USING lv_file
    ls_table .
    FREE it_KNB1 .
    CLEAR lt_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_T042Z-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'LAND1' INTO ls_table.
    DESCRIBE FIELD wa_T042Z-LAND1
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'ZLSCH' INTO ls_table.
    DESCRIBE FIELD wa_T042Z-ZLSCH
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'TEXT1' INTO ls_table.
    DESCRIBE FIELD wa_T042Z-TEXT1
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'XEINZ' INTO ls_table.
    DESCRIBE FIELD wa_T042Z-XEINZ
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'XSCHK' INTO ls_table.
    DESCRIBE FIELD wa_T042Z-XSCHK
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'XWECH' INTO ls_table.
    DESCRIBE FIELD wa_T042Z-XWECH
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'BLART' INTO ls_table.
    DESCRIBE FIELD wa_T042Z-BLART
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'XSWEC' INTO ls_table.
    DESCRIBE FIELD wa_T042Z-XSWEC
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'XORB' INTO ls_table.
    DESCRIBE FIELD wa_T042Z-XORB
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_T042Z = lv_T042Z + LINES( it_T042Z ) .
  IF lv_T042Z GT 0.
    PERFORM conv_table_to_text TABLES it_T042Z
    USING lv_file
    ls_table .
    FREE it_T042Z .
    CLEAR lt_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_T042ZT-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'SPRAS' INTO ls_table.
    DESCRIBE FIELD wa_T042ZT-SPRAS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'LAND1' INTO ls_table.
    DESCRIBE FIELD wa_T042ZT-LAND1
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'ZLSCH' INTO ls_table.
    DESCRIBE FIELD wa_T042ZT-ZLSCH
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'TEXT2' INTO ls_table.
    DESCRIBE FIELD wa_T042ZT-TEXT2
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_T042ZT = lv_T042ZT + LINES( it_T042ZT ) .
  IF lv_T042ZT GT 0.
    PERFORM conv_table_to_text TABLES it_T042ZT
    USING lv_file
    ls_table .
    FREE it_T042ZT .
    CLEAR lt_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_T16FT-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'SPRAS' INTO ls_table.
    DESCRIBE FIELD wa_T16FT-SPRAS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'FRGGR' INTO ls_table.
    DESCRIBE FIELD wa_T16FT-FRGGR
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'FRGSX' INTO ls_table.
    DESCRIBE FIELD wa_T16FT-FRGSX
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'FRGXT' INTO ls_table.
    DESCRIBE FIELD wa_T16FT-FRGXT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_T16FT = lv_T16FT + LINES( it_T16FT ) .
  IF lv_T16FT GT 0.
    PERFORM conv_table_to_text TABLES it_T16FT
    USING lv_file
    ls_table .
    FREE it_T16FT .
    CLEAR lt_table .
...
*&      Form  SUB_DOWNLOAD_/TMWFLOW/IMPSTAN
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
STATICS : lv_no_head TYPE c.
  ...
  lv_file = 'TMWFLOW_IMPSTAN.ACTT'.
  IF lv_no_head IS INITIAL.

    lv_no_head = 'X'.

    ls_table = 'SUB_LANDSCAPE_ID'.
    DESCRIBE FIELD wa_/TMWFLOW/IMPSTAN-SUB_LANDSCAPE_ID
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'BRANCH_NAME' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/IMPSTAN-BRANCH_NAME
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'CYCLE_TYPE' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/IMPSTAN-CYCLE_TYPE
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'SYS_NAME' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/IMPSTAN-SYS_NAME
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'SYS_TYPE' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/IMPSTAN-SYS_TYPE
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'SYS_CLIENT' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/IMPSTAN-SYS_CLIENT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'SYS_ROLE' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/IMPSTAN-SYS_ROLE
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'PROC_TYPE' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/IMPSTAN-PROC_TYPE
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'STSMA' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/IMPSTAN-STSMA
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'STATUS' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/IMPSTAN-STATUS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_/TMWFLOW/IMPSTAN = lv_/TMWFLOW/IMPSTAN + LINES( it_/TMWFLOW/IMPSTAN ) .
  IF lv_/TMWFLOW/IMPSTAN GT 0.
    PERFORM conv_table_to_text TABLES it_/TMWFLOW/IMPSTAN
    USING lv_file
    ls_table .
    FREE it_/TMWFLOW/IMPSTAN .
    CLEAR lt_table .
    CLEAR ls_table .
  ELSE.
    APPEND ls_table TO lt_table .
...
*  <--  p2        text
*----------------------------------------------------------------------*
FORM SUB_DOWNLOAD_/TMWFLOW/PROJCTC.
STATICS : lv_no_head TYPE c.

  lv_file = 'TMWFLOW_PROJCTC.ACTT'.
  IF lv_no_head IS INITIAL.

    lv_no_head = 'X'.

    ls_table = 'TASKLIST'.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTC-TASKLIST
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'PROJECT_TYPE' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTC-PROJECT_TYPE
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'SMI_PROJECT' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTC-SMI_PROJECT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'PARENT_TASKLIST' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTC-PARENT_TASKLIST
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'RESP_USER' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTC-RESP_USER
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'CREATED_DATE' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTC-CREATED_DATE
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'STATUS' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTC-STATUS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'ORIGINATOR_KEY' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTC-ORIGINATOR_KEY
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_/TMWFLOW/PROJCTC = lv_/TMWFLOW/PROJCTC + LINES( it_/TMWFLOW/PROJCTC ) .
  IF lv_/TMWFLOW/PROJCTC GT 0.
    PERFORM conv_table_to_text TABLES it_/TMWFLOW/PROJCTC
    USING lv_file
    ls_table .
    FREE it_/TMWFLOW/PROJCTC .
    CLEAR lt_table .
    CLEAR ls_table .
  ELSE.
    APPEND ls_table TO lt_table .
...
*&      Form  SUB_DOWNLOAD_/TMWFLOW/PROJCTT
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
STATICS : lv_no_head TYPE c.
  ...
  lv_file = 'TMWFLOW_PROJCTT.ACTT'.
  IF lv_no_head IS INITIAL.

    lv_no_head = 'X'.

    ls_table = 'LANGU'.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTT-LANGU
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'TASKLIST' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTT-TASKLIST
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'DESCRIPTION' INTO ls_table.
    DESCRIBE FIELD wa_/TMWFLOW/PROJCTT-DESCRIPTION
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.

  ENDIF.
  lv_/TMWFLOW/PROJCTT = lv_/TMWFLOW/PROJCTT + LINES( it_/TMWFLOW/PROJCTT ) .
  IF lv_/TMWFLOW/PROJCTT GT 0.
    PERFORM conv_table_to_text TABLES it_/TMWFLOW/PROJCTT
    USING lv_file
    ls_table .
    FREE it_/TMWFLOW/PROJCTT .
    CLEAR lt_table .
    CLEAR ls_table .
  ELSE.
    APPEND ls_table TO lt_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_T043-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'USNAM' INTO ls_table.
    DESCRIBE FIELD wa_T043-USNAM
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'RFPRO' INTO ls_table.
    DESCRIBE FIELD wa_T043-RFPRO
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'INVPR' INTO ls_table.
    DESCRIBE FIELD wa_T043-INVPR
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_T043 = lv_T043 + LINES( it_T043 ) .
  IF lv_T043 GT 0.
    PERFORM conv_table_to_text TABLES it_T043
    USING lv_file
    ls_table .
    FREE it_T043 .
    CLEAR lt_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_T683U-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'SPRAS' INTO ls_table.
    DESCRIBE FIELD wa_T683U-SPRAS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'KVEWE' INTO ls_table.
    DESCRIBE FIELD wa_T683U-KVEWE
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'KAPPL' INTO ls_table.
    DESCRIBE FIELD wa_T683U-KAPPL
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'KALSM' INTO ls_table.
    DESCRIBE FIELD wa_T683U-KALSM
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'VTEXT' INTO ls_table.
    DESCRIBE FIELD wa_T683U-VTEXT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_T683U = lv_T683U + LINES( it_T683U ) .
  IF lv_T683U GT 0.
    PERFORM conv_table_to_text TABLES it_T683U
    USING lv_file
    ls_table .
    FREE it_T683U .
    CLEAR lt_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_T685T-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'SPRAS' INTO ls_table.
    DESCRIBE FIELD wa_T685T-SPRAS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'KVEWE' INTO ls_table.
    DESCRIBE FIELD wa_T685T-KVEWE
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'KAPPL' INTO ls_table.
    DESCRIBE FIELD wa_T685T-KAPPL
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'KSCHL' INTO ls_table.
    DESCRIBE FIELD wa_T685T-KSCHL
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'VTEXT' INTO ls_table.
    DESCRIBE FIELD wa_T685T-VTEXT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_T685T = lv_T685T + LINES( it_T685T ) .
  IF lv_T685T GT 0.
    PERFORM conv_table_to_text TABLES it_T685T
    USING lv_file
    ls_table .
    FREE it_T685T .
    CLEAR lt_table .
...
STATICS : lv_no_head TYPE c.
  ...
  IF lv_no_head IS INITIAL.
    ...
    ls_table = 'MANDT'.
    DESCRIBE FIELD wa_CABNT-MANDT
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'ATINN' INTO ls_table.
    DESCRIBE FIELD wa_CABNT-ATINN
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'SPRAS' INTO ls_table.
    DESCRIBE FIELD wa_CABNT-SPRAS
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
    CONCATENATE ls_table '|^|' 'ADZHL' INTO ls_table.
    DESCRIBE FIELD wa_CABNT-ADZHL
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  CONCATENATE ls_table '|^|' 'ATBEZ' INTO ls_table.
    DESCRIBE FIELD wa_CABNT-ATBEZ
    TYPE lv_type LENGTH lv_len IN BYTE MODE.
    CONCATENATE ls_table ' [' lv_type '(' lv_len ')]' INTO ls_table.
  ENDIF.
  lv_CABNT = lv_CABNT + LINES( it_CABNT ) .
  IF lv_CABNT GT 0.
    PERFORM conv_table_to_text TABLES it_CABNT
    USING lv_file
    ls_table .
    FREE it_CABNT .
    CLEAR lt_table .
...
* end of change -- sv 20200224
    ...
    PERFORM fill_seltab USING 'CLIENT' 'S' 'I' 'EQ' '' ''.
    ...
    PERFORM fill_seltab USING 'EVENTS' 'P' 'I' 'EQ' 'AUO' ''.
    PERFORM fill_seltab USING 'EVENTS' 'P' 'I' 'EQ' 'AU1' ''.
    PERFORM fill_seltab USING 'EVENTS' 'P' 'I' 'EQ' 'AU2' ''.
    PERFORM fill_seltab USING 'EVENTS' 'P' 'I' 'EQ' 'AU3' ''.
    PERFORM fill_seltab USING 'EVENTS' 'P' 'I' 'EQ' 'AU4' ''.
...
* Build Header Line per requirement
        ...
        CONCATENATE ls_table 'UserId [C(00030)]|^|TerminalName [C(00040)]|^|Tcode [C(00030)]|^|Program' INTO ls_table .
        CONCATENATE ls_table ' [C(00080)]|^|SysLogMessageGroup [C(00004)]|^|SubName [C(00002)]|^|AuditClass' INTO ls_table.
        CONCATENATE ls_table ' [C(00050)]|^|SecurityLevel [C(00050)]|^|AuditLogMessageText [C(00300)]'  INTO ls_table SEPARATED BY space.
...
*** SV 02102020 -- SM21 EXTRACTION LOGIC
*&---------------------------------------------------------------------*
*&      Form  EXTRACT_SM20_2(Using RSAU_READ_LOG)
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
* end of change -- sv 20200224
    ...
    PERFORM fill_seltab USING 'P_SELE' 'P' 'I' 'EQ' '1' ''.
...
* Build the data with the needed delimiter .
      IF lv_lines GT 0 .
        LOOP AT lt_list_asci INTO list_asci.
          ...
          ENDIF.
* begin of change -- sv 20200224
          IF lv_cn1 IS INITIAL.
            CONTINUE.
          ENDIF.
* begin of change -- sv 20200224

          DATA : algdate1 TYPE string,
                             algtime1 TYPE string,
                             algclient1 TYPE string,
                             alguser1 TYPE string,
                             algterm1 TYPE string,
                             algtcode1 TYPE string,
                             algrepna1 TYPE string,
                             algarea1 TYPE string,
                             algsubid1 TYPE string,
                             txsubclsid1 TYPE string,
                             txseverity1 TYPE string,
                             algshortsev1 TYPE string,
                             algtext1 TYPE string,
                             rest1 TYPE string,rest2 TYPE string.


          SPLIT list_asci AT '|' INTO rest1 algdate1
                             algtime1
                             algclient1
                             alguser1
                             algterm1
          ...
          IF STRLEN( algdate1 ) EQ 10.
            CONCATENATE algdate1
                               algtime1
                               algclient1
                               alguser1
                               algterm1
                               ...
                               txseverity1
                               algshortsev1
                               algtext1 INTO ls_table SEPARATED BY '|^|' .
            APPEND ls_table TO lt_table .
          ENDIF.
          CLEAR ls_table .
        ENDLOOP.
...
ENDFORM.                    " EXTRACT_SM20(Using RSAU_READ_LOG)
...
FORM sub_download_acct_config.
  ...
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
  CONCATENATE 'Extract Script Version' '|^|' '21.0' INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
  ...
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
  CONCATENATE 'Extract Application Version' '|^|' 'ACTT SAP' INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
  CONCATENATE 'Number of tables extracted' '|^|' table_count INTO ls_table.
  APPEND ls_table TO lt_table .
  CLEAR ls_table.
  ...
  IF lv_exist IS NOT INITIAL
    ...
    'X' INTO ls_table .
      APPEND ls_table TO lt_table .
  CLEAR ls_table .
      CONCATENATE 'Audit Start Date' '|^|'
     s_start   INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table .
*#ACTTSAP12.0 Release 12.0 Changes End*
  CONCATENATE 'Audit End Date' '|^|'
 s_end INTO ls_table .
  APPEND ls_table TO lt_table .
  CLEAR ls_table .
  ELSE.
...
*** pimittal -- SM51 EXTRACTION LOGIC
*&---------------------------------------------------------------------*
*&      Form  EXTRACT_SM51_NEW
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
FORM extract_sm51_NEW .
  ...
  PERFORM check_file USING 'SM51.ACTT'.
...
*    IF lv_lines GT maxsize OR lv_index EQ lv_num.
      ...
      IF lv_head IS INITIAL.
* Build Header Line per requirement
        CONCATENATE ls_table 'App_Server_Inst [C(00100)]|^|HOST [C(00100)]|^|' INTO ls_table .
        CONCATENATE ls_table 'Service [C(00300)]|^|Status [C(00040)]' INTO ls_table SEPARATED BY space.
        APPEND ls_table TO lt_table .
        CLEAR ls_table. lv_head = 'X'.
      ENDIF.
...
*          WITH SELECTION-TABLE gt_seltab
    ...
    DESCRIBE TABLE list_asci LINES lv_lines.
    IF lv_lines GT 0.
          DELETE list_asci INDEX lv_lines  .  "Delete last three lines contains blank line and no data.
        lv_lines = lv_lines - 1 .
        DELETE list_asci INDEX lv_lines .          "Delete last three lines contains blank line and no data.
       lv_lines = lv_lines - 1 .
        DELETE list_asci INDEX lv_lines .          "Delete last three lines contains blank line and no data.

        DELETE list_asci INDEX 3 .          "Delete first 3 lines, which contain the header and horizontal lines.
          DELETE list_asci INDEX 2 .          "Delete first 3 lines, which contain the header and horizontal lines.
              DELETE list_asci INDEX 1 .          "Delete first 3 lines, which contain the header and horizontal lines.
  ENDIF.
